...
**Passwordless CRM SSO (true SSO):** with `TWENTY_APP_SECRET` + SSO enabled, authenticated Flolah users open CRM on the **company workspace host** `{sub}.crm.<apex>` (top-level, first-party). Server-side `getAuthTokensFromLoginToken` proves membership; the browser gets a **short apply token** (`/flolah-handoff/?t=` → `/flolah-crm-sso`) that writes Twenty `tokenPairState` and opens `/`. Do **not** send `/verify?loginToken=` (iframe or nested query) — Twenty shows **Authentication failed** and `/welcome`. Backend JIT provision requires `TWENTY_DATABASE_URL` and writes `workspaceMember` into the workspace’s real `core.workspace.databaseSchema`. Company owners are provisioned as **Admin**.
