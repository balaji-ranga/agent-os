...
**Passwordless CRM SSO (true SSO):** with `TWENTY_APP_SECRET` + SSO enabled, authenticated Flolah users open CRM **in the Flolah iframe** on the company workspace host `{sub}.crm.<apex>` (same shell as ERP). Server-side `getAuthTokensFromLoginToken` proves membership; the iframe gets a **short apply token** (`/flolah-handoff/?t=` → `/flolah-crm-sso`) that writes Twenty `tokenPairState` and opens `/`. Do **not** send `/verify?loginToken=` — Twenty shows **Authentication failed** and `/welcome`. Do **not** navigate the Flolah window away from the CRM page. Backend JIT provision requires `TWENTY_DATABASE_URL` and writes `workspaceMember` into the workspace’s real `core.workspace.databaseSchema`. Company owners are provisioned as **Admin**. CRM open also detaches `userWorkspace` rows that still point at reclaimed/soft-deleted desks — Twenty 2.29 crashes CheckUserExists / GetCurrentUser on those rows (`/welcome` plus **An error occurred while checking user existence**). Reclaiming a desk detaches them too.
...
**SSO:** CRM menu → first-party Twenty workspace `/flolah-handoff/?t=` → `/flolah-crm-sso` (token pair apply, then `/`) **or** ERPNext desk (`/app/crm`) when CRM=ERPNext. ERP menu → `/flolah-erp-handoff/?t=…` → same-origin `/flolah-erp-sso` (Set-Cookie `sid` + 302 Desk) for company-scoped user. Requires nginx proxy for ERP `erp-sso-apply` and CRM `crm-sso-apply` (see `nginx.host-network.conf`).
