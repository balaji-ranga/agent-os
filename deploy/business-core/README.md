...
**Prefab agents:** Profile CRM = `twenty` → CRM Maker A/B + Checker (`crm_*` content tools) **only in that CEO's org**. Profile ERP = `erpnext` → ERP P&L / Invoice / Project Manager. Switching CRM or ERP away from platform provider **removes those prefab agents from the org** (agents remain in the platform catalog disabled for the CEO until re-selected).
