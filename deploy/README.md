...
| Company setup | `GET/POST/PUT /api/company-setup/*` (gate, funnel, design, apply, **`GET /blueprints?industry=`**, **`GET/PUT /company-memory`** for Update Company Details). Industry packs + admin publish as before. **`video_content`** and **`demo_balaji_ranganathan`** (`companion_packs: video_content`) install the tested Content Orchestrator + video graphs from `standard/video-content/`. UI: **/company-setup** + avatar **Update Company Details** `/update-company-details` → Knowledge `company_memory`. Help **29** + **35** + **41**. Distinct from Onboarding Helper **27**. |
