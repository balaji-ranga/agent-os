...
- `USER_API_KEYS_KEK` — wraps optional API Key vault encryption phrases (generate: `openssl rand -hex 32`). Non-platform Profiles auto-seed unset vault slots (`Platform_BYOK`, `Replicate_BYOK`, `BRAVE_SEARCH_BYOK`, `elevenlabs-key`); see platform-help **15**.
...
| Agent channels | Slack/WhatsApp BYOK wizard → vault + OpenClaw bindings; outbound **`MEDIA:`** attach; inbound → `inbound/attachments/` (OpenClaw `media/inbound` staging deleted after mirror); `/api/agent-channels`; platform-help **24**; `OPENCLAW_MEDIA_MAX_MB` (default 128) applied by `configure-openclaw-docker.js` |
