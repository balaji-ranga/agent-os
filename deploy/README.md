...
| Owner/agent-scoped content-tool credentials | backend startup | ✓ hot-loaded from shared OpenClaw volume |
...
5. Applies Docker overrides (`configure-openclaw-docker.js`) — gateway token and plugin `baseUrl`; removes legacy shared plugin keys
...
Business Core MCP proxies owner-scoped `crm_*` / `erp_*` content tools (`TOOLS_API_KEY` + `X-Ceo-User-Id`). Prefab **Maker/Checker** AI employees are provisioned when Profile selects Twenty/ERPNext — not when MCP starts (CRM agents still join the org if Twenty desk create hits the self-hosted workspace cap; unused Twenty desks from CRM≠Twenty or offboarded CEOs are reclaimed, never another living company's workspace). They are **domain SMEs**: CRM **Lead→Order** (help **40** + workspace **DOMAIN.md**); ERP **O2C/P2P** (help **39**). `master_data_rag` merges Flolah Help (`corpus=platform-help`) for those agents. ERPNext multi-tenant isolation: `flolah_company` masters + native `company` transactions; agents inherit CEO company scope. Multi-phase O2C uses durable **agent goal plans** (`agent_goal_*` async ack + new agr per create; scheduled **goal_run_plan** fire = new agr each tick; multiphase `agent_workflow_trigger` may upgrade; platform advances on async workflow terminal; **once-only completion chat nudge** + notify/watch correlate **agr + title**; Digest **2** + `/goal-plans`). Docs: `knowledgebase/platform-help/32-business-core-crm-erp.md`, **28**, **38**, **39**, **40**, `deploy/business-core/README.md`.
...
| `TOOL_API_RATE_LIMIT_RESET_CRON` | Daily audit+zero of per-user tool API **call** actuals at day/month roll (default `5 0 * * *`). UI: **Tools → Rate limits**. Validator also lazy-resets on the next call. |
| `EVENT_PRODUCTIVITY_RETRY_CRON` | Retry failed normalized productivity events with bounded backoff (default every minute); five failures move an item to the owner-scoped dead-letter inbox. |
| `SCHEDULED_GOALS_CRON` | Master tick for **Scheduled goals** (default `* * * * *`). Cadences: **hourly** \| daily \| weekdays \| weekly. Fires only **active** CEO prompts; paused/deleted stay off across restarts. UI: `/scheduled-goals` create/**edit**; **Generate draft is COO-only** (other employees **Save & schedule**). Optional **deliver_to** WhatsApp/Slack for the **final outcome**. COO tools `scheduled_goal_*`. Related: `GOAL_PLAN_*`, `SCHEDULED_GOAL_CHAT_TIMEOUT_MS`, `WORKFLOW_TERMINAL_WATCH_CRON` (all in compose backend-env). Docs: platform-help **28** / **42**. Verify: `vps-verify-scheduled-goals.sh`. |
| `CRM_TLS_WORKSPACE_CERT_CRON` | Hourly CRM workspace LE SAN expand when new Twenty `{sub}.crm.*` missing from cert (default `40 * * * *`; use `off` to disable). Post-create debounce: `CRM_TLS_WORKSPACE_CERT_AUTO`. Admin → Crons id `crm_tls_workspace_certs`. Docs: platform-help **19** / **32**. |
...
- `TOOLS_API_KEY` — transitional backend/internal-sidecar credential; never expose through OpenClaw or public clients
- `AGENT_OS_INTERNAL_TOKEN` — stable secret (auto-generated by `up.sh` / `ensure-deploy-secrets.js`)
- `USER_API_KEYS_KEK` — wraps optional API Key vault encryption phrases (generate: `openssl rand -hex 32`). Also encrypts **MCP OAuth App client secrets** (admin + CEO override) and related vaulted OAuth tokens. Non-platform Profiles auto-seed unset vault slots (`Platform_BYOK`, `Replicate_BYOK`, `BRAVE_SEARCH_BYOK`, `GOOGLE_PLACES_BYOK`, `X_API_BYOK`, `INSTAGRAM_SESSIONID`, `elevenlabs-key`); see platform-help **15** and **31**.
...
| Tools nav | CEO nav label **Tools** → `/content-tools` (content-tools plugin/API name unchanged); **Tools → Model** maps per-tool LLM models (BYOK-aware; owner-scoped `tool_model_overrides`); **Tools → Rate limits** maps per-user daily/monthly API call caps (`tool_api_rate_limits` + reset audit) |
...
| Scheduled goals | `GET/POST/PATCH /api/scheduled-goals` (+ pause/resume/run-now/plan); cadence **hourly**\|daily\|weekdays\|weekly; optional **`deliver_to`** WhatsApp/Slack (final outcome, `From: {agent}`); COO tools `scheduled_goal_*`; master tick `SCHEDULED_GOALS_CRON`. UI: **/scheduled-goals** create/**edit**; Generate draft **COO-only**. Home search: `GET /api/home/search` (task id + WF run id). Platform Help **28** / **42**. Verify: `bash deploy/scripts/vps-verify-scheduled-goals.sh` |
| Company setup | `GET/POST/PUT /api/company-setup/*` (gate, funnel, design, apply, **`GET /blueprints?industry=`**, **`GET/PUT /company-memory`** for Update Company Details). Industry packs + admin publish as before. **`video_content`** and **`demo_balaji_ranganathan`** (`companion_packs: video_content`) install the tested Content Orchestrator + video graphs from `standard/video-content/`. **`demo_education`** is the Meridian College deep pack (`publish-education-demo-blueprint.js`; do not regenerate `standard/business-core` from that owner). UI: **/company-setup** + avatar **Update Company Details** `/update-company-details` → Knowledge `company_memory`. Help **29** + **35** + **41**. Distinct from Onboarding Helper **27**. |
...
| `https://flolah.cloud/blog/` | Public blog; `/blog/forum/` discussion hub → GitHub Discussions |
...
- Runtime reconciliation preserves the current serialization whenever the effective permission set is unchanged; only real grant changes rewrite config
