...
| Company setup | `GET/POST/PUT /api/company-setup/*` (gate, funnel, design, apply, **`GET /blueprints?industry=`**, **`GET/PUT /company-memory`** for Update Company Setup). Industry packs + admin publish as before. UI: **/company-setup** + avatar **Update Company Setup** `/update-company-setup` → Knowledge `company_memory`. Help **29** + **35**. Distinct from Onboarding Helper **27**. |
