...
| Scheduled goals | `GET/POST /api/scheduled-goals` (+ pause/resume/run-now); COO tools `scheduled_goal_*`; master tick `SCHEDULED_GOALS_CRON` (default `* * * * *`). UI: **/scheduled-goals**. Platform Help **28**. Verify: `bash deploy/scripts/vps-verify-scheduled-goals.sh` |
