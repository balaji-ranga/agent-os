...
Business Core MCP proxies owner-scoped `crm_*` / `erp_*` content tools (`TOOLS_API_KEY` + `X-Ceo-User-Id`). Prefab **Maker/Checker** AI employees are provisioned when Profile selects Twenty/ERPNext — not when MCP starts. ERPNext multi-tenant isolation: `flolah_company` masters + native `company` transactions; agents inherit CEO company scope. Docs: `knowledgebase/platform-help/32-business-core-crm-erp.md`, `deploy/business-core/README.md`.
