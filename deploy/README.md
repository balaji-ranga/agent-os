...
| Agent channels | Slack/WhatsApp BYOK wizard → vault + OpenClaw bindings; outbound **`MEDIA:`** attach; inbound → `inbound/attachments/` (OpenClaw `media/inbound` staging deleted after mirror). WhatsApp replies prepend **`From: {employee name}`** (`channels.whatsapp.responsePrefix` + `identity.name`). Scheduled-goal WhatsApp copies send `MEDIA:` TTS as a **follow-up voice note** (not text-only). WhatsApp **`groupPolicy` defaults to `disabled`** so `@g.us` group traffic is rejected before media download (DM `allowFrom` alone does not cover groups; set allowlist/open in Channels wizard if intentional). `/api/agent-channels`; platform-help **24**; `OPENCLAW_MEDIA_MAX_MB` (default 128) applied by `configure-openclaw-docker.js` |
