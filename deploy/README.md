...
| Scheduled goals | `GET/POST /api/scheduled-goals` (+ pause/resume/run-now); COO tools `scheduled_goal_*`; master tick `SCHEDULED_GOALS_CRON` (default `* * * * *`). UI: **/scheduled-goals**. Verify: `bash deploy/scripts/vps-verify-scheduled-goals.sh` |
