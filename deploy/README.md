...
| Scheduled goals | `GET/POST/PATCH /api/scheduled-goals` (+ pause/resume/run-now/plan); cadence **hourly**\|daily\|weekdays\|weekly; COO tools `scheduled_goal_*`; master tick `SCHEDULED_GOALS_CRON`. UI: **/scheduled-goals** create/**edit**/amend plan. Home search: `GET /api/home/search` (task id + WF run id). Platform Help **28**. Verify: `bash deploy/scripts/vps-verify-scheduled-goals.sh` |
