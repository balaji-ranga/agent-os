...
| Company setup | `GET/POST/PUT /api/company-setup/*` (gate, funnel, design, apply, **`GET /blueprints?industry=`**); industry packs = JSON under `backend/src/services/company-blueprints/packs/` + admin-published rows in `company_industry_blueprints`. UI: **/company-setup** (default blueprint + slide to pick variants). Admin: **/admin** → Company industry blueprints (`GET/POST /api/admin/company-blueprints*`, **`GET …/:id/export.zip`** = v2 pack: knowledge, policies, org, agents+tools+ops MD, workflow graphs, connectors without secrets, goals). Publish code: `company-blueprint-publish.js` + `registry.js`. Re-snapshot/validate: `node scripts/validate-company-blueprint-export.mjs` / `republish-content-ops-blueprint.js`. Content Creator scope: FB/IG/LI/blogs only (YouTube deferred). Platform Help **29**. Distinct from Onboarding Helper **27**. |
