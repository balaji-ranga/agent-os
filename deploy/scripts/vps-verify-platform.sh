...
check "welcome video embed" grep -q '/docs/media/13-northstar-ai-native-company.mp4' "$ROOT/docs-site/docs/start/welcome.md"
check "welcome video source" test -s "$ROOT/knowledgebase/video-tours/assets/13-northstar-ai-native-company.mp4"
check "welcome video published" test -s "$ROOT/deploy/static/flolah-home/docs/media/13-northstar-ai-native-company.mp4"
check "welcome captions published" test -s "$ROOT/deploy/static/flolah-home/docs/media/13-northstar-ai-native-company.vtt"
check "public docs architecture" test -f "$ROOT/docs-site/docs/start/how-the-company-runs.mdx"
check "public docs IBKRNew event trader" test -f "$ROOT/docs-site/docs/systems/ibkrnew-event-trader.md"
...
check "CRM embed iframe" grep -q '<iframe' "$ROOT/frontend/src/pages/BusinessEmbed.jsx"
check "CRM embed not top-nav" test -z "$(grep -F 'twentyTopNavOnce' "$ROOT/frontend/src/pages/BusinessEmbed.jsx" || true)"
...
check "agent actions owner-scoped routes" test -f "$ROOT/backend/src/routes/agent-actions.js"
check "agent actions live endpoint" grep -q "'/live'" "$ROOT/backend/src/routes/agent-actions.js"
check "orchestrator agent schema" grep -q 'is_orchestrator' "$ROOT/backend/src/db/schema.js"
check "goal-step isolated delegation session" grep -q 'delegationSessionUserForPrompt' "$ROOT/backend/src/services/delegation-queue.js"
check "non-retryable quota classification" grep -q 'quota_exhausted' "$ROOT/backend/src/services/tool-failure-class.js"
...
check "public docs sensitive scan script" test -f "$ROOT/backend/scripts/scan-public-docs-sensitive.js"
...
check "help doc: IBKRNew event trader" test -f "$ROOT/knowledgebase/platform-help/51-ibkrnew-event-trader.md"
check "help catalog IBKRNew event trader" grep -q '51-ibkrnew-event-trader.md' "$ROOT/backend/src/services/ceo-default-master-data.js"
...
check "browser multi-executor schema" grep -q "browser_executor_nodes" "$ROOT/backend/src/db/schema.js"
check "Flolah Chrome extension source" test -f "$ROOT/backend/flolah-chrome-extension/manifest.json"
check "Flolah extension package service" test -f "$ROOT/backend/src/services/flolah-chrome-extension-package.js"
check "compose browser extension flag" grep -q "BROWSER_EXTENSION_ENABLED" "$ROOT/deploy/docker-compose.yml"
...
check "OpenClaw session cleanup service" test -f "$ROOT/backend/src/services/openclaw-session-cleanup.js"
check "compose injects OpenClaw session cleanup policy" grep -q 'OPENCLAW_SESSION_CLEANUP_DRY_RUN' "$ROOT/deploy/docker-compose.yml"
check "OpenClaw cleanup safety test" test -f "$ROOT/backend/scripts/test-openclaw-session-cleanup.mjs"
