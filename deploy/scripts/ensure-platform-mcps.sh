...
upsert WEB_SCRAPE_MCP_URL 'http://web-scrape-mcp:8085/mcp'
...
# ---- Platform MCPs (Compose profiles optional-brave-mcp + optional-meta-graph-mcp + optional-business-core-mcp + optional-social-research-mcp + optional-web-scrape-mcp) ----
# ensure-platform-mcps.sh starts containers + seeds mcp-brave-search / mcp-meta-graph / mcp-flolah-crm / mcp-flolah-erp / mcp-social-research / mcp-web-scrape (is_platform=1).
...
# Web scrape: Crawlee sidecar (Cheerio / Playwright); workflow Web Scrape node + scrape_url / scrape_domain. Docs: knowledgebase/platform-help/44-web-scrape.md
...
if [[ -f "$ROOT/tools/web-scrape-mcp/server.js" ]]; then
  if [[ "$PLATFORM_MCP_BUILD" == "1" ]]; then
    docker compose --env-file "$ENV_FILE" --profile optional-web-scrape-mcp build "${BUILD_ARGS[@]}" web-scrape-mcp \
      || echo "ensure-platform-mcps: WARN web-scrape-mcp build failed"
  fi
  docker compose --env-file "$ENV_FILE" --profile optional-web-scrape-mcp up -d --force-recreate web-scrape-mcp \
    || echo "ensure-platform-mcps: WARN web-scrape-mcp up failed"
else
  echo "ensure-platform-mcps: WARN missing tools/web-scrape-mcp/server.js"
fi

# Wait for MCP health endpoints (non-fatal)
for svc_port in "brave-search-mcp:8080" "meta-graph-mcp:8081" "business-core-mcp:8082" "social-research-mcp:8084" "web-scrape-mcp:8085"; do
  svc="${svc_port%%:*}"
  port="${svc_port##*:}"
  ok=0
...
if [[ -f "$ROOT/backend/scripts/seed-web-scrape-mcp.js" ]]; then
  echo "==> seed mcp-web-scrape (platform)"
  docker compose --env-file "$ENV_FILE" exec -T \
    -e WEB_SCRAPE_MCP_URL="${WEB_SCRAPE_MCP_URL:-http://web-scrape-mcp:8085/mcp}" \
    backend node scripts/seed-web-scrape-mcp.js \
    || echo "ensure-platform-mcps: WARN seed-web-scrape-mcp failed"
fi
