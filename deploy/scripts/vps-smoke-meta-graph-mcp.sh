...
echo "==> verify registry row"
docker compose --env-file .env exec -T backend node --input-type=module -e '
import { initDb, getDb } from "./src/db/schema.js";
initDb();
const s = getDb().prepare("SELECT id, is_platform, owner_role, status FROM mcp_servers WHERE id=?").get("mcp-meta-graph");
const c = getDb().prepare("SELECT provider, enabled FROM mcp_oauth_configs WHERE server_id=?").get("mcp-meta-graph");
if (!s || !s.is_platform || s.owner_role !== "admin") throw new Error("mcp-meta-graph not platform: " + JSON.stringify(s));
if (!c || !c.enabled) throw new Error("oauth config missing: " + JSON.stringify(c));
