...
function snapshotExpression(limit) {
  return `(() => {
    ...
    const elements=[],landmarks=[],seenRoots=new WeakSet(),seenElements=new WeakSet();
    const queue=[{root:document,offsetX:0,offsetY:0}];
    while(queue.length&&elements.length<max){
      const item=queue.shift(),root=item.root;
      if(!root||seenRoots.has(root))continue;seenRoots.add(root);
      for(const el of root.querySelectorAll(q)){if(elements.length>=max)break;if(seenElements.has(el))continue;seenElements.add(el);const r=el.getBoundingClientRect(),view=el.ownerDocument?.defaultView||window,cs=view.getComputedStyle(el);if(r.width<=0||r.height<=0||cs.display==='none'||cs.visibility==='hidden')continue;let id=el.getAttribute('data-flolah-ref');if(!id){id=String(++s.counter);el.setAttribute('data-flolah-ref',id)}const type=String(el.getAttribute('type')||'').toLowerCase();const editable=el.matches('input,textarea,[contenteditable="true"]');const sensitive=type==='password'||/password|secret|token|card number|cvv/i.test(String(el.getAttribute('aria-label')||el.getAttribute('name')||''));const value=editable&&!sensitive?String(('value' in el?el.value:(el.innerText||el.textContent||''))).trim().slice(0,4000):'';const dialog=el.closest('[role="dialog"],[aria-modal="true"]');elements.push({ref:'g'+s.generation+'-e'+id,role:el.getAttribute('role')||({A:'link',BUTTON:'button',INPUT:'textbox',TEXTAREA:'textbox',SELECT:'combobox'}[el.tagName]||el.tagName.toLowerCase()),name:String(el.getAttribute('aria-label')||el.innerText||el.getAttribute('placeholder')||el.getAttribute('name')||'').trim().slice(0,180),value,enabled:!el.disabled&&el.getAttribute('aria-disabled')!=='true',visible:true,editable,sensitive,focused:el.ownerDocument?.activeElement===el,in_dialog:!!dialog,dialog_name:dialog?String(dialog.getAttribute('aria-label')||dialog.getAttribute('name')||'').slice(0,120):'',bounds:{x:Math.round(r.x+item.offsetX),y:Math.round(r.y+item.offsetY),width:Math.round(r.width),height:Math.round(r.height)}})}
      for(const el of root.querySelectorAll('main,nav,header,footer,form,[role="main"],[role="navigation"],[role="dialog"]')){if(landmarks.length>=80)break;landmarks.push({role:el.getAttribute('role')||el.tagName.toLowerCase(),name:String(el.getAttribute('aria-label')||el.getAttribute('name')||'').slice(0,120)})}
      for(const el of root.querySelectorAll('*')){if(el.shadowRoot)queue.push({root:el.shadowRoot,offsetX:item.offsetX,offsetY:item.offsetY});if(el.tagName==='IFRAME'){try{const doc=el.contentDocument;if(doc){const r=el.getBoundingClientRect();queue.push({root:doc,offsetX:item.offsetX+r.x,offsetY:item.offsetY+r.y})}}catch{}}}
    }
...
async function act(tabId, request) {
  ...
  const target = local ? `(() => { if(Number(globalThis.__flolahSnapshotState?.generation||0)!==${generation})return 'STALE_REF';const roots=[document],seen=new WeakSet();while(roots.length){const root=roots.shift();if(!root||seen.has(root))continue;seen.add(root);const found=root.querySelector('[data-flolah-ref="${local.replace(/"/g, '')}"]');if(found)return found;for(const el of root.querySelectorAll('*')){if(el.shadowRoot)roots.push(el.shadowRoot);if(el.tagName==='IFRAME'){try{if(el.contentDocument)roots.push(el.contentDocument)}catch{}}}}return null;})()` : 'null';
  if (kind === 'click') {
    let point = null;
    let identity = null;
    if (local) {
