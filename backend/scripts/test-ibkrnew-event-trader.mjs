...
const eligibilityCredentials = service.registerBridge(eligibilityOwner, 'DU-ELIGIBILITY'); const eligibilityBridge = service.authenticateBridge(eligibilityCredentials.bridge_id, eligibilityCredentials.token);
...
service.ingestBridgeEvent(approvalBridge, { event_id: 'approval-9', sequence: 9, event_type: 'desktop.component_error', occurred_at: new Date().toISOString(), payload: { component_id: 'IBKRNewGateway', component_type: 'ibkr_gateway', code: 'TEST_DISCONNECT', message: 'test gateway disconnect' } });
