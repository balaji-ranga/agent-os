...
try {
  ...
  const withCooOpinion = addReviewOpinion({ ownerUserId: 'owner-review', reviewId: review.id, evidenceId: 'goal-fail', actorRole: 'coo', agentId: 'coo', position: 'agree_with_revisions', content: 'Preserve partial results, but stop retrying quota failures.' }); assert.equal(withCooOpinion.opinions[0].position, 'agree_with_revisions');
  addReviewOpinion({ ownerUserId: 'owner-review', reviewId: review.id, evidenceId: 'goal-fail', actorRole: 'agent', agentId: 'coo', position: 'acknowledge', content: 'I can stop retrying non-transient quota failures and retain partial evidence.' });
