...
try {
  ...
  assert.ok(companyRegistry.sources.find((source) => source.id === 'flolah_crm').formulas.some((item) => item.id === 'selected_pipeline'));
  const template = companyRegistry.rule_templates[0];
  companyRegistry = svc.upsertMeasurementRegistryEntry('ceo-demo-northstar', template);
  assert.equal(companyRegistry.rules[0].validation.schema_valid, true);
  assert.equal(companyRegistry.rules[0].validation.evaluator_connected, false);
  svc.deleteMeasurementRegistryEntry('ceo-demo-northstar', 'composite_rule', template.id);
  svc.deleteMeasurementRegistryEntry('ceo-demo-northstar', 'formula', 'selected_pipeline');
