...
try {
  ...
  const customRegistry = await request('/api/company-objectives/measurement-registry', { method: 'PUT', body: JSON.stringify({ kind: 'source', id: 'warehouse', label: 'Company warehouse', provider: 'SQL' }) }); assert.equal(customRegistry.response.status, 200); assert.ok(customRegistry.body.sources.some((source) => source.id === 'warehouse'));
  ...
  const goalIdea = await request('/api/company-objectives/ideate-goals', { method: 'POST', body: JSON.stringify({ use_llm: false, objective: idea.body.proposal, initiative: idea.body.proposal.initiatives[0], key_results: idea.body.proposal.key_results }) }); assert.equal(goalIdea.response.status, 200); assert.equal(goalIdea.body.goals.length, 2);
  const scheduledInitiatives = idea.body.proposal.initiatives.slice(0, 3).map((initiative, index) => ({ ...initiative, cadence: 'Continuous', goals: [{ id: `ig-api-${index}`, goal_type: 'scheduled', enabled: true, title: `${initiative.name} cycle`, prompt: initiative.prompt, cadence: index === 2 ? 'daily' : 'weekdays', time_local: ['09:00','14:00','17:00'][index], timezone: 'Asia/Singapore', linked_key_result_ids: [idea.body.proposal.key_results[0].id] }, ...(index===0?[{ id: 'ig-api-adhoc', goal_type: 'adhoc', enabled: true, title: 'Initial research sprint', prompt: 'Create initial research evidence.', linked_key_result_ids: [idea.body.proposal.key_results[1].id] }]:[])] }));
  ...
  assert.equal(objective.initiatives[1].goals[0].time_local, '14:00');
  assert.equal(objective.initiatives[0].goals.length, 2, 'an initiative stores multiple scheduled/ad-hoc goal definitions');
