  ...
  return [
    ...
    },
    {
      ...
      lines: [
        ...
        '- **speech_tts** / **speech_stt** / **analyze_image**: Free Piper TTS and Whisper STT when the CEO asks to speak or transcribe (optional-voice). Use **analyze_image** for inbound images (describe / OCR / thumbnail review) with `inbound/attachments/<file>` or MEDIA: path — chat models are often text-only so you must call this tool. WhatsApp/web media is mirrored to `inbound/attachments/<file>` — use that relative path (or MEDIA:/root/.openclaw/media/inbound/…) with speech_stt, analyze_image, or the Summarize inbound media attachment workflow. If you see “[whatsapp attachment unavailable]” (or Telegram equivalent): (1) call **list_inbound_attachments**, (2) pick the newest matching `relative_path`, (3) call **analyze_image** for images or **speech_stt** / summarize workflow for audio/video. Do not stop at “unavailable” — bytes usually land in inbound within a few seconds.',
