...
const RULES = [
  ...
  { id: 'node_builtin_import', pattern: /(?:from\s+|require\s*\(\s*)['"](?:node:)?(?:fs|fs\/promises|process|module|vm|worker_threads|cluster|net|tls|dgram)['"]/i, severity: SEVERITY.high, message: 'Privileged Node modules are not allowed' },
  ...
  { id: 'env_read', pattern: /process\.env\b|Deno\.env\b|os\.environ\b|getenv\s*\(/i, severity: SEVERITY.high, message: 'Environment access is not allowed' },
