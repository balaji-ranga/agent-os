...
function upsertStep(runId, node, status, extra = {}) {
  ...
  const existing = appendOnly
    ...
    : db()
        .prepare(`SELECT id, status FROM agent_workflow_run_steps WHERE run_id = ? AND node_id = ? AND iteration = 1`)
        .get(runId, node.id);
  ...
  if (existing) {
    ...
    maybeNotifyWorkflowStep(runId, node, status, prevStatus);
    return existing.id;
  }
  db()
    .prepare(
  ...
  updateRunProgress(runId);
  maybeNotifyWorkflowStep(runId, node, status, null);
  return stepId;
}

function maybeNotifyWorkflowStep(runId, node, status, prevStatus) {
  try {
    if (!nodeWantsNotification(node)) return;
    if (status === prevStatus) return;
    let phase = null;
    if (status === 'in_progress' || status === 'listening') phase = 'started';
    else if (status === 'completed') phase = 'completed';
    else if (status === 'failed') phase = 'failed';
    else return;
    if (phase === 'started' && prevStatus === 'in_progress') return;

    const run = db()
      .prepare(
        `SELECT r.id, r.run_number, r.owner_user_id, r.definition_id, d.name AS definition_name
         FROM agent_workflow_runs r
         LEFT JOIN agent_workflow_definitions d ON d.id = r.definition_id
         WHERE r.id = ?`
      )
      .get(runId);
    if (!run?.owner_user_id) return;
    notifyWorkflowNodeEvent({
      ownerUserId: run.owner_user_id,
      runId: run.id,
      runNumber: run.run_number,
      definitionId: run.definition_id,
      definitionName: run.definition_name || '',
      node,
      phase,
    });
  } catch (e) {
    console.warn('[agent-workflow] notify step failed:', e?.message || e);
  }
}
