...
export function getOauthConfigRow(serverId) {
  return (
    getDb()
      .prepare('SELECT * FROM mcp_oauth_configs WHERE server_id = ?')
      .get(String(serverId || '').trim()) || null
  );
}

export function getOauthConfigPublic(serverId) {
  const row = getOauthConfigRow(serverId);
  ...
  return {
    ...
    client_secret_configured: !!(envOr(row.client_secret, row.client_secret_env) || row.client_secret),
...
export function listOauthCandidateServers(authUser) {
    ...
    rows = db
      .prepare(
        `SELECT s.id, s.name, s.description, s.url, s.status, s.is_platform, s.owner_role, s.owner_user_id,
         ...
         FROM mcp_servers s
         LEFT JOIN mcp_oauth_configs c ON c.server_id = s.id
  ...
  return rows.map((r) => {
    ...
    const secretOk = !!(String(r.client_secret || '').trim() || envOr('', r.client_secret_env));
    return {
      server_id: r.id,
      name: r.name,
...
export function upsertOauthConfig(serverId, body = {}, authUser) {
  ...
  if (!isPlatformAdmin(authUser) && !allowOwner) {
    throw Object.assign(
      new Error('Only platform admin can configure platform MCP OAuth; CEOs may enable OAuth on their own MCP servers'),
      { status: 403 }
    );
  }
  const id = String(serverId || body.server_id || '').trim();
  ...
  const existing = getOauthConfigRow(id);
  const provider = String(body.provider || existing?.provider || 'oauth2').trim().toLowerCase();
  const defaults = providerDefaults(provider);

  const authorization_url = String(
    body.authorization_url ?? body.authorizationUrl ?? existing?.authorization_url ?? defaults.authorization_url ?? ''
  ).trim();
  const token_url = String(body.token_url ?? body.tokenUrl ?? existing?.token_url ?? defaults.token_url ?? '').trim();
  ...
  if (body.client_secret != null || body.clientSecret != null) {
    ...
    if (next) client_secret = next;
  }

  const client_id_env = String(
    body.client_id_env ?? body.clientIdEnv ?? existing?.client_id_env ?? defaults.client_id_env ?? ''
  ).trim();
  const client_secret_env = String(
    body.client_secret_env ?? body.clientSecretEnv ?? existing?.client_secret_env ?? defaults.client_secret_env ?? ''
  ).trim();

  const scopes = String(body.scopes ?? existing?.scopes ?? defaults.scopes ?? '').trim();
  const display_name = String(
    body.display_name ?? body.displayName ?? existing?.display_name ?? defaults.display_name ?? id
  ).trim();
  const enabled =
  ...
  const auth_header_name = String(
    body.auth_header_name ?? body.authHeaderName ?? existing?.auth_header_name ?? 'Authorization'
  ).trim();
  const auth_header_template = String(
    body.auth_header_template ??
      body.authHeaderTemplate ??
      existing?.auth_header_template ??
      'Bearer {{access_token}}'
  ).trim();
  const token_request_style = String(
    body.token_request_style ?? body.tokenRequestStyle ?? existing?.token_request_style ?? defaults.token_request_style ?? 'form'
  ).trim();
  const refresh_enabled =
    body.refresh_enabled === false || body.refreshEnabled === false
      ...
      : body.refresh_enabled === true || body.refreshEnabled === true
        ? 1
        : existing?.refresh_enabled != null
          ? existing.refresh_enabled
            ? 1
            : 0
          : 1;

  const extra =
    body.extra_auth_params ?? body.extraAuthParams ?? parseJson(existing?.extra_auth_params_json, {});
  const provider_options =
    body.provider_options ?? body.providerOptions ?? parseJson(existing?.provider_options_json, defaults.provider_options || {});
  ...
  return getOauthConfigPublic(id);
}
...
export function resolveOauthClientCredentials(cfg) {
  ...
  const clientSecret = envOr(cfg.client_secret, cfg.client_secret_env);
  if (!clientId || !clientSecret) {
    throw Object.assign(
      new Error(
        `OAuth client not configured for MCP ${cfg.server_id}. Admin must set client id/secret (or env ${cfg.client_id_env || 'CLIENT_ID'}/${cfg.client_secret_env || 'CLIENT_SECRET'}).`
      ),
      { status: 503 }
    );
  }
...
export function listOauthConnectorsForUser(authUser) {
  ...
  return configs.map((c) => {
    ...
    const clientId = envOr(c.client_id, c.client_id_env);
    return {
      ...
      scopes: c.scopes || '',
      oauth_client_ready: !!clientId && !!(envOr(c.client_secret, c.client_secret_env) || c.client_secret),
...
export function startMcpOauth(serverId, authUser) {
  ...
  const cfg = getOauthConfigRow(id);
  if (!cfg || !cfg.enabled) {
...
export async function handleMcpOauthCallback({ code, state, error, error_description } = {}) {
  ...
  const cfg = getOauthConfigRow(row.server_id);
  if (!cfg) {
...
export function getMcpOauthAuthHeaders(ownerUserId, serverId) {
  const cfg = getOauthConfigRow(serverId);
...
export function deleteOauthConfig(serverId, authUser) {
  ...
  getDb().prepare('DELETE FROM mcp_oauth_configs WHERE server_id = ?').run(String(serverId || '').trim());
  return { ok: true };
}
