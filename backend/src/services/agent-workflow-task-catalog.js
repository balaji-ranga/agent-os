...
export const WORKFLOW_TASK_TYPES = {
  ...
  message_send: {
    ...
    color: '#0f766e',
    inputs: [
      { id: 'destination', label: 'Destination override', required: false, mode: 'static', description: 'Queue, topic, subject or channel. Defaults to node configuration.' },
      { id: 'payload', label: 'Message payload', required: true, mode: 'dynamic', description: 'Text or JSON from a prior step.' },
      { id: 'headers', label: 'Headers (JSON)', required: false, mode: 'static', placeholder: '{}' },
      { id: 'key', label: 'Partition/routing key', required: false, mode: 'static' },
    ],
    outputs: [
      { id: 'ok', label: 'Published' },
      { id: 'message_id', label: 'Broker message ID' },
      { id: 'result', label: 'Publish receipt' },
      { id: 'text', label: 'Summary' },
    ],
    configFields: withNodeTimeoutConfigFields([
      { id: 'connectionId', label: 'Messaging connection', type: 'text' },
      { id: 'destination', label: 'Destination', type: 'text' },
      { id: 'destinationType', label: 'Destination type', type: 'select', options: ['queue', 'topic'], default: 'queue' },
      { id: 'headersJson', label: 'Headers (JSON)', type: 'textarea', placeholder: '{}' },
      { id: 'key', label: 'Routing / partition key', type: 'text' },
      { id: 'correlationId', label: 'Correlation ID', type: 'text' },
      { id: 'replyTo', label: 'Reply destination', type: 'text' },
      { id: 'qos', label: 'MQTT QoS', type: 'number', default: 1 },
      { id: 'persistent', label: 'Persistent delivery', type: 'boolean', default: true },
      { id: 'ttlMs', label: 'TTL (ms)', type: 'number' },
      { id: 'partition', label: 'Kafka partition', type: 'number' },
    ]),
  },
  externalAgent: {
    type: 'externalAgent',
  ...
  filesystem: {
    ...
    inputs: [
      {
        ...
        required: true,
        mode: 'static',
        description: 'Windows (C:\\data\\file.txt), Unix (/var/data/file.txt), or remote FTP/SFTP path',
      },
      {
        id: 'glob',
        label: 'Glob (list)',
      ...
      {
        id: 'content',
        label: 'Content (write_text)',
        required: false,
        mode: 'dynamic',
        description: 'Text to write; bind from an upstream node',
      },
      {
        id: 'destination',
    ...
    configFields: [
      ...
      },
      {
        ...
        label: 'Operation',
        type: 'select',
        options: ['list', 'exists', 'stat', 'read_text', 'write_text', 'move'],
        default: 'list',
      },
      { id: 'path', label: 'Default path', type: 'text', placeholder: 'C:\\data\\inbox or /var/data/inbox' },
      { id: 'glob', label: 'Default glob', type: 'text', default: '*', placeholder: '*.txt' },
      { id: 'destination', label: 'Default destination (move)', type: 'text' },
      { id: 'maxBytes', label: 'Max read/write bytes', type: 'number', default: 65536 },
      { id: 'host', label: 'FTP/SFTP host', type: 'text' },
      { id: 'port', label: 'FTP/SFTP port', type: 'number' },
      { id: 'username', label: 'FTP/SFTP user', type: 'text' },
      {
        id: 'executeOn',
        label: 'Execute on (desktop package)',
        type: 'select',
        options: ['auto', 'local', 'server'],
        default: 'auto',
      },
    ],
  },
