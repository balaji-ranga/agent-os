 ...
 * Keep in sync with frontend/src/utils/isoCountryRegion.js
 */
...
/** ERPNext Country doctype uses short English names, not always ISO official names. */
const ERPNEXT_COUNTRY_NAME = {
  US: 'United States',
  GB: 'United Kingdom',
...
export function regionBelongsToCountry(regionCode, countryAlpha2) {
  const sub = parseSubdivision(regionCode, countryAlpha2);
  return !!(sub && sub.country === String(countryAlpha2 || '').toUpperCase());
}

/**
 * Map stored/legacy free text to ISO codes. Unmapped text yields empty country.
 */
...
/**
 * Strict write path: empty/global allowed; otherwise ISO 3166 only (legacy names coerced).
 */
export function normalizeCountryRegion(input = {}) {
  const countryRaw = input.country == null ? '' : String(input.country).trim();
  const regionRaw = input.region == null ? '' : String(input.region).trim();
  if (
    (!countryRaw && !regionRaw) ||
    (isGlobalLocationToken(countryRaw) && !regionRaw) ||
    (!countryRaw && isGlobalLocationToken(regionRaw)) ||
    (isGlobalLocationToken(countryRaw) && isGlobalLocationToken(regionRaw))
  ) {
    return { country: '', region: '' };
  }
  const parsed = parseIsoLocation(countryRaw, regionRaw);
  if (!parsed.country) {
    const err = new Error('country must be an ISO 3166-1 alpha-2 code from the country list');
    err.status = 400;
    throw err;
  }
  if (regionRaw && !isGlobalLocationToken(regionRaw) && !parsed.region && parseCountryCode(regionRaw) !== parsed.country) {
    const err = new Error('region must be an ISO 3166-2 subdivision of the selected country');
    err.status = 400;
    throw err;
  }
  return parsed;
...
export function erpnextCountryName(countryAlpha2) {
  const cc = String(countryAlpha2 || '').trim().toUpperCase();
  if (!cc) return '';
  if (ERPNEXT_COUNTRY_NAME[cc]) return ERPNEXT_COUNTRY_NAME[cc];
  const row = getIsoCountry(cc);
  return row ? row.name : '';
}
