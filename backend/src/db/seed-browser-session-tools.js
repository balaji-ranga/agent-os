...
const BROWSER_TOOLS = [
  ...
  },
  {
    ...
    method: 'POST',
    purpose:
      'API tool: prepare or run a saved browser recipe for this CEO. For draft/prepare/preview/do-not-publish requests, pass prepare_only:true with recipe_name or recipe_id and all inferred inputs; it validates bindings and never creates a browser task. For execution, omit prepare_only, pass operation/action_effect from the exact effect contract documented by browse_task_start, supply every required input under inputs, and optionally wait_ms up to 90000. When the CEO requires a specific executor, pass preferred_driver and allow_fallback:false; excluded_drivers is also supported. Missing inputs are rejected before task creation. Multiple placeholders are passed together, e.g. {"recipe_name":"Social post","operation":"social_publish","inputs":{"post_content":"Exact text","first_comment":"Exact comment"}}. This tool does not add blanket approval: follow the existing action policy and the CEO request. Do not use exec.',
    model_used: '',
    enabled: 1,
    is_builtin: 1,
  },
