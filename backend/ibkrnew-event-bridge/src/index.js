...
setInterval(async () => {
  try { const gatewayHealth = gateway?.health() || { connected: false }; core.emit('bridge.heartbeat', { bridge_version: '1.1.0', gateway_connected: gatewayHealth.connected, mode: mock ? 'paper_mock' : 'paper', spool_depth: core.spoolDepth(), components: [{ component_id: 'IBKRNewDesktopRuntime', component_type: 'desktop_runtime', status: 'online', version: process.version }, { component_id: 'IBKRNewDurableSpool', component_type: 'event_spool', status: 'online', depth: core.spoolDepth() }, { component_id: 'IBKRNewGateway', component_type: 'ibkr_gateway', status: gatewayHealth.connected ? 'online' : 'offline', ...gatewayHealth }] }); await core.flush(); for (const command of await core.claim(10)) { if (!gatewayHealth.connected) continue; const seen = core.commandSeen(command.command_id); if (seen) { await core.acknowledge(command.command_id, 'uncertain', { reason: 'durable_command_journal_reclaim', prior: seen }); continue; } try { const executionBoot = await core.bootstrap(); if (!commandMatchesBootstrap(command, executionBoot)) throw new Error('account reference epoch changed before execution'); core.markCommand(command.command_id, 'executing'); const detail = await gateway.placeProtected(command); core.markCommand(command.command_id, 'submitted', detail); await core.acknowledge(command.command_id, 'submitted', detail); } catch (e) { core.markCommand(command.command_id, 'rejected', { error: e.message }); core.emit('desktop.component_error', { component_id: 'IBKRNewExecutionAdapter', component_type: 'execution_adapter', code: 'COMMAND_REJECTED', message: e.message, command_id: command.command_id }); await core.acknowledge(command.command_id, 'rejected', { error: e.message }); } } }
