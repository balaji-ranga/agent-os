...
3. Multi-intent prompts get a **draft execution plan** (workflows + specialty + notify). On the page: **Generate draft plan** → **Amend plan manually** (or **Build plan manually**) if intent→step mapping is wrong → **Save draft** / **Approve plan & schedule**. COO tools auto-approve. **Each schedule fire (plan mode)** creates a **new** `agr-…` from the approved step template (not chat reuse of yesterday's plan). Ad-hoc multiphase in chat: COO **`agent_goal_create`** (**async ack** — plan + `agr-…`, platform advances steps; new multiphase asks create a **new** plan unless CEO says status/continue). Digest shows last **2** plans (with run timestamps) → **View all** `/goal-plans`.
...
| **Video content studio** | Industry pack `video_content`: **Content Orchestrator**; phrases **run video storyboard** / **run video media** / **run video assembly**. S4 clips ≤8s/scene (`flow_browser` or `replicate_api`); S5 FFmpeg → status `video_generated`. Help **41**. |
...
| **Connectors** | **OpenConnector** (OAuth / API key / no-auth + optional CEO App override — help **16**) + **MCPs** (help **31**). Downloads: **Browser Session package** (multi-user local worker, help **22**, BROWSER-SESSION-DESKTOP-LOCAL.md) and **Local IBKR bridge** (help **20**). |
