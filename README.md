...
| **Business Core (CRM/ERP)** | Optional **Twenty** or **ERPNext** CRM / **ERPNext** ERP on Profile or Company setup. CRM=Twenty → Twenty SSO + Maker/Checker (`crm_*`). CRM=ERPNext → ERPNext desk SSO + sales `erp_*`. ERP=ERPNext → Maker A/B, Checker, P&L/Invoice/Project specialists. **Option 1 coordination:** Kanban control plane; ERP Checker hard-submit; CRM high-risk process gate; optional seed workflows (`run erp/crm maker checker`). **COO:** company-scoped **read-only** list/report `crm_*`/`erp_*`. **Platform Help:** RAG docs only (**38–40**), never live books. **Tenant isolation:** 1 CEO → 1 ERPNext Company; global masters use `flolah_company`; transactions force `company`; desk User UP + agents share CEO scope; fiscal years redact peers; org sync optional. **SSO JIT:** after Twenty membership SQL, backend invalidates Redis member flat-maps (`TWENTY_REDIS_URL`) so CRM REST does not `FORBIDDEN` on new workspace members. MCP `mcp-flolah-crm`/`mcp-flolah-erp`. Docs: platform-help **32**/**38–40**, `deploy/business-core/README.md`. | **Source packs:** \ackend/src/services/company-blueprints/standard/; demo company pack packs/demo_balaji_ranganathan.json (Balaji Ranganathan / BalajiDemoCompany export)\ (agents + CRM/ERP MC graphs; IBKR manifest). Industry packs: \packs/\.
