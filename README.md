...
8. Optional (content creator): **Connectors → MCPs** for Facebook Page OAuth; publish / community / weekly ops rollup (help **30**). Ops rollup uses the **bell** (`notify_ceo`), not email.
...
| **Content creator ops** | Company Operate + `content_creator` pack: production loop, **Facebook Page** publish via platform MCP **`mcp-meta-graph`** (Connectors → MCPs OAuth; Page posts only), OpenConnector for other SaaS, **content-comments-ingest** + community triage, Channel Publisher **`agent_workflow_trigger`** → `content-publish-social`. Help: platform-help **30**. Operators: `ensure-platform-mcps.sh`, optional `SEED_CONTENT_MEDIA_OWNER`. |
