...
3. Multi-intent prompts get a **draft execution plan** (workflows + specialty steps + notify). Approve on the page, or use COO tools (auto-approve). Multiphase CRM→ERP also works as **ad-hoc** `agent_goal_create` in chat (`agr-…`); Digest shows the last **2** plans for the week → **View all** `/goal-plans`.
...
| **Scheduled goals + goal plans** | Management → **Scheduled goals** (`/scheduled-goals`): recurring CEO prompts (**hourly** / daily / weekdays / weekly; perpetual or end date). Create/**edit**/list/pause/run-now or COO `scheduled_goal_*`. Multi-intent / multiphase prompts use durable **`agent_goal_runs`** (`agr-…`) with platform step advance on workflow/specialty terminal (stored plans keep workflow phrases; residual notify_ceo / Platform Help do not drop specialty steps). **Draft plan** flow on create (CEO UI approve) or COO auto-approve. Ad-hoc multiphase chat: COO **`agent_goal_create`** (also list/status/complete_step). Multiphase CRM+ERP language on **`agent_workflow_trigger`** auto-upgrades to a goal plan when no `goal_run_id` is bound — a workflow integer **`run_id` is never a goal plan**. Inspect: Digest **2** plans + **View all** `/goal-plans`, chat Goal Plan panel, `agent_goal_*`, `GET /api/agent-goal-runs`. Env: `GOAL_PLAN_MAX_SPECIALTY`, `SCHEDULED_GOALS_CRON`, `WORKFLOW_COO_WAKE_ON_TERMINAL`. Help **28** / **38**, content pack **30**. E2E: `cd backend && npm run test:e2e:goal-plan` (included in full regression pack; skip with `REGRESSION_GOAL_PLAN=0`). |
