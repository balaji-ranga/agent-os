...
| **Scheduled goals** | Management → **Scheduled goals** (`/scheduled-goals`): recurring CEO prompts (**hourly** / daily / weekdays / weekly; perpetual or end date). Create/**edit**/list/pause/run-now in the UI or via COO tools (`scheduled_goal_*`). Multi-intent prompts (e.g. CRM then ERP maker-checker phrases) fire in **goal plan mode** (`agent_goal_run` + platform advance on workflow terminal); inspect with `agent_goal_list` / `agent_goal_status`. Only **active** rows fire; pause/delete survives restarts. Hourly uses `time_local` minutes (`:MM`). Platform master tick `SCHEDULED_GOALS_CRON`. **Ops Reporter** weekly rollup goals use **`notify_ceo` → bell** (not email). Help: platform-help **28** / **38**, content pack **30**. |
...
| **Business Core (CRM/ERP)** | Optional **Twenty** or **ERPNext** CRM / **ERPNext** ERP on Profile or Company setup. CRM=Twenty -> Twenty SSO + Maker/Checker (`crm_*`). CRM=ERPNext -> ERPNext desk SSO + sales `erp_*`. ERP=ERPNext -> Maker A/B, Checker, P&L/Invoice/Project specialists. **Option 1 coordination:** Kanban control plane; ERP Checker hard-submit; CRM high-risk process gate; seed workflows (`run erp/crm maker checker`) with Maker `needs_ceo` -> **ceo_approval** for e.g. 5% discount (not free-form CEO Kanban HITL). **COO:** company-scoped **read-only** list/report; **`agent_workflow_trigger` is async** (ack `run_id`, platform notify on CEO-wait/terminal). **Platform Help:** RAG docs only (**38-40**), never live books. **Tenant isolation:** 1 CEO -> 1 ERPNext Company; global masters use `flolah_company`; transactions force `company`; desk User UP + agents share CEO scope; fiscal years redact peers; org sync optional. **SSO JIT:** after Twenty membership SQL, backend invalidates Redis member flat-maps (`TWENTY_REDIS_URL`) so CRM REST does not `FORBIDDEN` on new workspace members. MCP `mcp-flolah-crm`/`mcp-flolah-erp`. Docs: platform-help **32**/**38-40**, `deploy/business-core/README.md`. **Source packs:** `backend/src/services/company-blueprints/standard/` (agents + CRM/ERP MC graphs; IBKR manifest). Industry packs: `packs/`. |
