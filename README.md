...
| **Digest (This Week)** | Top-nav `/this-week`: Time Saved, Est. Value Delivered (sum of completed work x each AI employee `hourly_rate_usd`, hire default $10/hr), **2 most recent goal plans** for the week (**View all** → `/goal-plans`), insights. Env: `THIS_WEEK_MINUTES_PER_TASK`, fallback `THIS_WEEK_VALUE_USD_PER_HOUR`. COO tool `this_week_digest`. Platform Help **02**. |
