...
| **Business Core (CRM/ERP)** | Optional **Twenty** or **ERPNext** CRM / **ERPNext** ERP on Profile or Company setup. CRM=Twenty → Twenty SSO + Maker/Checker (`crm_*`). CRM=ERPNext → ERPNext desk SSO + sales `erp_*`. ERP=ERPNext → Maker A/B, Checker, P&L/Invoice/Project specialists. **Tenant isolation:** 1 CEO → 1 ERPNext Company; global masters use `flolah_company`; transactions force `company`; desk User UP + agents share CEO scope; fiscal years redact peers; org sync → Department + Employee. MCP `mcp-flolah-crm`/`mcp-flolah-erp`. Docs: platform-help **32**, `deploy/business-core/README.md`. |
