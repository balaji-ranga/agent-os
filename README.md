...
| **Business Core (CRM/ERP)** | Optional **Twenty** CRM / **ERPNext** ERP on Profile or Company setup. Selecting platform CRM provisions CRM Maker A/B + Checker; platform ERP provisions **ERP P&L**, **Invoice**, and **Project Manager** specialists with `crm_*` / `erp_*` tools, passwordless embed SSO (CRM LOGIN token; ERP `/flolah-erp-handoff` sid), nav embeds when configured, and platform MCP for workflows. Plan: `knowledgebase/BUSINESS-CORE-WORKSPACE-PLAN.md`. Daily **Work** surface: `/work`. ERP Docker: MariaDB multi-company site (`optional-erpnext`); isolation 1 CEO → 1 Company. |
