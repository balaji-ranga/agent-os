...
- **Deploy repair:** `deploy/scripts/ensure-openclaw-gateway-config.js` restores wiped `gateway` / `tools` / `plugins` / `browser` **and** an empty **model catalog**. `vps-verify-openclaw-chat.sh` re-runs `configure-openclaw-docker.js` when the catalog is empty (Agent Chat **502** `Unknown model: openai/…`). Default cloud slug is `openai/deepseek-v4-flash`. For **free local** OpenClaw + platform LLM: `APPLY_LOCAL_OLLAMA=1 bash deploy/scripts/ensure-local-openclaw-ollama.sh` (wanted `mistral-medium-3.5` 128B; CPU 16GB uses `llama3.2`; never Ollama Cloud). Catalog `OLLAMA_CONTEXT_WINDOW=65536`, runtime `OLLAMA_NUM_CTX=32768` (8B@65k OOM → 408). See [knowledgebase/LOCAL-OPENCLAW-OLLAMA.md](knowledgebase/LOCAL-OPENCLAW-OLLAMA.md).
