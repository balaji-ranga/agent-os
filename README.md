...
| **Video content studio** | Industry pack `video_content`: chat **Content Orchestrator**, phrase **run video storyboard** (Story → cast CEO gate with portraits → Scene → Prompt → storyboard CEO gate). Character faces: generate via `video_characters_ensure_refs` or CEO upload → name → `video_characters_bind_upload` into Master Data. Help **41**. |
