...
1. State the **outcome** in one message when you can (**tell the COO the outcome first** — count, deadline, budget, what must not happen). Do not open Workflow Builder until you inspect a Goal Plan step. Routine step retries and provider fallbacks do not need a follow-up message; only policy, auth, or a scope change will ask you.
...
- **Speak** checkbox — Piper plays the **full** assistant reply in chunks (no audio attachment in the thread).
- **Phone icon** — live WebRTC when **Channels → Voice** is enabled on that employee (COO included). Slow Caller without Voice does not show Call.

See [Channels](../systems/channels.md) and [Hire AI employees](../setup/hire-ai-employees.md).

## How the COO delegates

Clear specialty asks (“research X”, “draft a social post”) are handed to the matching employee with full context. Vague “help me” may stay with the COO.

The COO creates **Kanban** cards for specialty work. Watch the board and the **bell** when something needs you.

COO-native asks (list workflows, standups, tools, Kanban) usually stay with the COO.

If you want a specialist to **reach you**, say so (“have Research contact me”). That employee rings the bell with a link to **their** chat. Ordinary live replies should not spam the bell — ask them to **notify you** when async work finishes.
...
A scored example of telling the COO the outcome first (40 verified prospects, $75 cap, no invented contacts, no unapproved send): [Example stress test run](../operate/example-stress-test-run.md).
