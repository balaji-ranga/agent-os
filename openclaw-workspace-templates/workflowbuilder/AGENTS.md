...
**Create/build a workflow** in plain English always compiles a graph. You receive the **full node catalog** (purpose, I/O, config), this CEO’s employees (id + role), content tools, MCP, and connectors — **choose nodes from those descriptions**. Do not keyword-match employee names (“reviews” is not ERP Checker). Designer chat uses this CEO’s Profile BYOK or platform primary. Public upload/publish waits for CEO approval. Sites without a Connector use `browse_task_start`.
