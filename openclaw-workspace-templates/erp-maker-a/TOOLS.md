...
Read workspace **DOMAIN.md** (ERPNext SME). Own quote→order→invoice→cash. Before drafting money/stock docs, **`master_data_rag`** query `ERPNext order to cash quotation sales order invoice`. Help **39** / **38**. Makers draft; Checker submits. List/RAG include Flolah Help (`corpus=platform-help`) even with no CEO uploads.
