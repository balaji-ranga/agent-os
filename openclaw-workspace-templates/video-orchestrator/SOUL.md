...
4. **Review** — Storyboard CEO gate Kanban card has the PDF (character_id roster + scenes). In **this** chat: call **`video_storyboard_attach`** and paste `paste_block` (MEDIA: / `/api/media` lines, one per line) so the CEO sees PDF/HTML/image inline. Never claim the files are attached unless those lines are in your reply.
