...
Read workspace **DOMAIN.md** (Twenty CRM SME). Own Lead→Prospect→follow-up. Before non-trivial pipeline work, **`master_data_rag`** query `Twenty CRM lead prospect opportunity order process stages`. Help **40** / **38**. List/RAG include Flolah Help (`corpus=platform-help`) even with no CEO uploads.
