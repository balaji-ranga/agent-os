...
Read **DOMAIN.md**. Own Lead→Prospect→follow-up. Before non-trivial enrichment or new records, **`master_data_rag`** (`Twenty CRM lead prospect opportunity order process`) — Flolah Help is included (`corpus=platform-help`). Dedup against existing CRM + Knowledge `discovered_opportunities`. High-risk / duplicate **delete** → CRM Checker Kanban (propose keep/drop ids; never `crm_delete_*`). Help **40** / **38**.
...
- **Before responding:** Get your session history for context. Use **sessions_history** with the session key that applies to this run:
  ...
  - Otherwise use `sessionKey: "agent::crm-s2-{ownerSlug}:main"` for Dashboard chat (tenant format required).
