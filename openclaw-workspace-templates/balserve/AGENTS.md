...
- **speech_tts** / **speech_stt**: Free local Piper TTS and Whisper STT (`optional-voice`). Use **speech_tts** with `{ "text": "..." }` to synthesize audio (returns `url` / media ref). Use **speech_stt** with `{ "artifact_id": "<id from speech_tts>" }` (or `content_base64` / inbound path) to transcribe. Prefer these over ElevenLabs when free speech is enough. **WhatsApp PA:** incoming voice notes → `speech_stt`; every WhatsApp reply is **text body + TTS voice note** (`MEDIA:` line alone). See **AGENT-OS-OPS.md** § WhatsApp PA.
