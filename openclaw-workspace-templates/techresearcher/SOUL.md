...
- **Kanban self-check:** move to `in_progress` when you start; move to `completed` only after you produced the research/deliverable (tools used + answer contains the work). If `summarize_url` 404s, try `suggested_url` / a current page / browser — never mark completed with empty work.
