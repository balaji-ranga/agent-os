  ...
  const createTable = async (e) => {
    ...
    try {
      const columns = newTableCols
        ...
        .map((c) => c.trim())
        .filter(Boolean);
      await api.masterDataTableCreate({ name: newTableName.trim(), columns });
      setNewTableName('');
      await refresh();
      flash('Table created.');
    } catch (err) {
  ...
  return (
    <div style={{ padding: '1.5rem', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <p style={{ color: 'var(--muted)', marginTop: 0 }}>
        Create tables, upload CSV, and store documents under your tenant data folder. Workflows use the{' '}
        <strong>Master Data</strong> node (query / RAG) with your entitlements only.
      </p>
