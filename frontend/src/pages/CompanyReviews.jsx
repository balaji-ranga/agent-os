...
function Overview({ review, refresh, setStage, selectEvidence }) {
  ...
  return <>
    <div className="review-period"><div><b>▣ {fmt(review.period_start)} – {fmt(review.period_end)}</b><span>Prepared by COO</span><mark>● {review.status === 'ready' ? 'Ready for review' : review.status.replace('_', ' ')}</mark></div>{review.status === 'completed' ? <span className="review-locked">🔒 Snapshot locked</span> : <button onClick={start}>{review.status === 'in_session' ? '▷ Continue review' : '▷ Start review'}</button>}</div>
...
function Session({ review, selected, setSelected, refresh, setStage }) {
  ...
  return <div className="review-session-layout">
    ...
    <section className="review-session-main">
      ...
      <div className="review-feedback"><label>Overall rating<select disabled={locked} value={rating} onChange={e=>setRating(e.target.value)}><option value="needs_improvement">Needs improvement</option><option value="meets_expectations">Meets expectations</option><option value="exceeds_expectations">Exceeds expectations</option></select></label><label>CEO feedback<textarea disabled={locked} value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Describe the outcome expected next time…" /></label><label>Learning destination<select disabled={locked} value={destination} onChange={e=>setDestination(e.target.value)}>{DESTINATIONS.map(d=><option key={d[0]} value={d[0]}>{d[1]}</option>)}</select></label><button disabled={locked || feedback.trim().length < 20} onClick={propose}>✦ Propose improvement</button></div>
    </section>
