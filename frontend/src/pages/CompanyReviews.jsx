...
function Session({ review, selected, setSelected, refresh, setStage }) {
  ...
  async function propose() {
    ...
    await api.companyReviewsFeedback(review.id, { evidence_type: evidence.type || 'goal', evidence_id: evidence.id, agent_id: evidence.agent_id, rating, feedback, classification: 'reusable_operating_lesson', scope: [evidence.agent_id].filter(Boolean) });
    await api.companyReviewsCreateImprovement(review.id, { title: `Improve: ${evidence.title}`, problem: evidence.error || `${evidence.retries || 0} retries / ${evidence.attention_steps || 0} attention steps`, proposed_change: feedback, destination, scope: [evidence.agent_id].filter(Boolean), evidence: [{ type: evidence.type || 'goal', id: evidence.id }], owner_agent_id: evidence.agent_id || review.prepared_by_agent_id || '', success_metric: 'Fewer repeat failures on comparable goals', validation_test: 'Evaluate on the next comparable real goal run' });
  ...
  return <div className="review-session-layout">
    ...
    <section className="review-session-main">
      <div className="review-evidence-picker"><label>Evidence under review</label><select value={evidence?.id || ''} onChange={(e)=>setSelected(allEvidence.find(x=>x.id===e.target.value))}>{allEvidence.map(x=><option key={x.id} value={x.id}>{x.type==='agent_contribution'?'Agent contribution':'Goal outcome'} · {x.agent_id || 'Attribution unavailable'} · {x.title} · {x.status}</option>)}</select></div>
      {evidence ? <><div className="review-outcome"><div><mark>{evidence.type==='agent_contribution'?'Agent contribution':evidence.success?'Completed outcome':'Needs improvement'}</mark><h2>{evidence.title}</h2><strong>{evidence.completed_steps}/{evidence.step_count} steps completed</strong></div><div className="cause-cards"><span><b>{evidence.retries || 0}</b>Retries</span><span><b>{evidence.attention_steps || 0}</b>Blocked/failed</span><span><b>{evidence.agent_id || 'Unknown'}</b>{evidence.type==='agent_contribution'?'Performer':'Orchestrator'}</span></div></div>
      ...
      <div className="review-opinion-actions"><div><h3>Review the CEO’s exact draft</h3><p>Enter CEO feedback below. Flolah then asks the COO and affected agent whether they agree with that exact text in separate, goal-isolated sessions.</p></div></div>{opinionError&&<div className="error-banner">{opinionError}</div>}
      ...
      <div className="review-feedback"><label>Overall rating<select disabled={locked} value={rating} onChange={e=>setRating(e.target.value)}><option value="needs_improvement">Needs improvement</option><option value="meets_expectations">Meets expectations</option><option value="exceeds_expectations">Exceeds expectations</option></select></label><label>CEO proposed feedback<textarea disabled={locked} value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Describe the specific behavior expected next time…" /><small className={feedback.trim().length<20?'review-field-warning':''}>{feedback.trim().length<20?`${20-feedback.trim().length} more characters required`:'Ready for COO and agent assessment'}</small></label><label>Learning destination<select disabled={locked} value={destination} onChange={e=>setDestination(e.target.value)}>{DESTINATIONS.map(d=><option key={d[0]} value={d[0]}>{d[1]}</option>)}</select></label><div className="review-feedback-actions"><button disabled={locked||opinionsLoading||feedback.trim().length<20} onClick={generateOpinions}>{opinionsLoading?'Requesting opinions…':evidenceOpinions.length?'↻ Reassess this CEO draft':'✦ Ask COO & affected agent'}</button><button disabled={locked || feedback.trim().length < 20 || !evidenceOpinions.some(x=>x.actor_role==='coo') || !evidenceOpinions.some(x=>x.actor_role==='agent')} onClick={propose}>✦ Propose governed improvement</button>{feedback.trim().length>=20&&!evidenceOpinions.length&&<small>Request both assessments before proposing.</small>}</div></div>
    </section>
    <aside className="review-speakers"><h3>Speaking order</h3><div className="done"><b>✓</b><span><strong>COO briefing</strong><small>Evidence and outcomes prepared</small></span></div><div className="speaking"><b>2</b><span><strong>Agent briefing</strong><small>{evidence?.agent_explanation || `${evidence?.agent_id || 'Assigned agent'} has no captured execution explanation.`}</small></span></div><div><b>3</b><span><strong>CEO feedback</strong><small>{locked ? 'Review completed' : 'Guidance pending'}</small></span></div></aside>
  </div>;
}
