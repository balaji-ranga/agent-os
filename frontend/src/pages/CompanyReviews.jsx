...
function Session({ review, selected, setSelected, refresh, setStage }) {
  ...
  const [cooPosition,setCooPosition]=useState('agree_with_revisions'); const [cooOpinion,setCooOpinion]=useState(''); const [agentPosition,setAgentPosition]=useState('acknowledge'); const [agentResponse,setAgentResponse]=useState('');
  ...
  async function saveOpinion(actorRole) {
    const isCoo=actorRole==='coo'; const content=isCoo?cooOpinion:agentResponse; const position=isCoo?cooPosition:agentPosition;
    if(!content.trim()||!evidence)return;
    await api.companyReviewsOpinion(review.id,{evidence_id:evidence.id,actor_role:actorRole,agent_id:isCoo?(review.prepared_by_agent_id||''):(evidence.agent_id||''),position,content}); await refresh();
    if(isCoo)setCooOpinion('');else setAgentResponse('');
  }
  const evidenceOpinions=(review.opinions||[]).filter(x=>x.evidence_id===evidence?.id);
  return <div className="review-session-layout">
    ...
    <section className="review-session-main">
      ...
      <div className="review-opinion-grid"><section><h3>COO assessment</h3>{evidenceOpinions.filter(x=>x.actor_role==='coo').map(x=><blockquote key={x.id}><b>{x.position.replaceAll('_',' ')}</b>{x.content}</blockquote>)}<select disabled={locked} value={cooPosition} onChange={e=>setCooPosition(e.target.value)}><option value="agree">Agree</option><option value="agree_with_revisions">Agree with revisions</option><option value="disagree">Disagree</option><option value="more_evidence">Need more evidence</option></select><textarea disabled={locked} value={cooOpinion} onChange={e=>setCooOpinion(e.target.value)} placeholder="COO's evidence-backed opinion on the CEO feedback…"/><button disabled={locked||!cooOpinion.trim()} onClick={()=>saveOpinion('coo')}>Record COO opinion</button></section><section><h3>Affected agent response</h3>{evidenceOpinions.filter(x=>x.actor_role==='agent').map(x=><blockquote key={x.id}><b>{x.position.replaceAll('_',' ')}</b>{x.content}</blockquote>)}<select disabled={locked} value={agentPosition} onChange={e=>setAgentPosition(e.target.value)}><option value="acknowledge">Acknowledge</option><option value="agree">Agree</option><option value="disagree">Disagree</option><option value="more_evidence">Need more evidence</option></select><textarea disabled={locked} value={agentResponse} onChange={e=>setAgentResponse(e.target.value)} placeholder="Agent explanation, constraints, or disagreement…"/><button disabled={locked||!agentResponse.trim()} onClick={()=>saveOpinion('agent')}>Record agent response</button></section></div>
      <div className="review-feedback"><label>Overall rating<select disabled={locked} value={rating} onChange={e=>setRating(e.target.value)}><option value="needs_improvement">Needs improvement</option><option value="meets_expectations">Meets expectations</option><option value="exceeds_expectations">Exceeds expectations</option></select></label><label>CEO final feedback<textarea disabled={locked} value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Consider the COO opinion and agent response, then describe the outcome expected next time…" /></label><label>Learning destination<select disabled={locked} value={destination} onChange={e=>setDestination(e.target.value)}>{DESTINATIONS.map(d=><option key={d[0]} value={d[0]}>{d[1]}</option>)}</select></label><button disabled={locked || feedback.trim().length < 20} onClick={propose}>✦ Propose governed improvement</button></div>
    </section>
