...
function Session({ review, selected, setSelected, refresh, setStage }) {
  ...
  async function generateOpinions() { if(!evidence)return; setOpinionsLoading(true); setOpinionError(''); try { await api.companyReviewsGenerateOpinions(review.id,evidence.id); await refresh(); } catch(error) { setOpinionError(error.message); } finally { setOpinionsLoading(false); } }
  ...
  return <div className="review-session-layout">
    ...
    <section className="review-session-main">
      ...
      <div className="review-opinion-actions"><div><h3>Independent review perspectives</h3><p>Flolah asks the COO and affected agent in separate, goal-isolated sessions. Their responses are read-only and cannot be authored by the CEO.</p></div><button disabled={locked||opinionsLoading} onClick={generateOpinions}>{opinionsLoading?'Requesting assessments…':evidenceOpinions.length?'↻ Refresh COO & agent assessments':'✦ Ask COO & affected agent'}</button></div>{opinionError&&<div className="error-banner">{opinionError}</div>}
