...
function Overview({ review, refresh, setStage, selectEvidence }) {
  ...
  async function start() { await api.companyReviewsSetStatus(review.id, 'in_session'); await refresh(); setStage('session'); }
  return <>
    <div className="review-period"><div><b>▣ {fmt(review.period_start)} – {fmt(review.period_end)}</b><span>Prepared by COO</span><mark>● {review.status === 'ready' ? 'Ready for review' : review.status.replace('_', ' ')}</mark></div><button onClick={start}>▷ Start review</button></div>
    ...
    <div className="review-overview-grid">
      ...
      <section className="review-evidence-column candidates"><h3><span>↗</span>Improvement candidates</h3>{(review.snapshot?.improvement_candidates || []).map((item) => <button type="button" key={item.id} className="review-candidate" onClick={() => { selectEvidence(item); setStage('session'); }}><b>{item.title}</b><small>{item.reason}</small><em>Goal run <i>›</i></em></button>)}</section>
    </div>
  </>;
}

function Session({ review, selected, setSelected, refresh, setStage }) {
  const evidence = selected || review.snapshot?.misses?.[0] || review.snapshot?.wins?.[0];
  const [feedback, setFeedback] = useState(''); const [rating, setRating] = useState('needs_improvement'); const [destination, setDestination] = useState('agent_playbook');
  ...
  return <div className="review-session-layout">
    <aside className="review-agenda"><h3>Review agenda</h3>{['Outcomes','Wins','Misses & blockers','Agent feedback','Commit improvements'].map((x,i)=><div className={i===2?'active':''} key={x}><b>{i+1}</b><span>{x}<small>{['Review committed outcomes','Celebrate what went well','Analyze gaps and causes','Hear from the agent','Agree on actions'][i]}</small></span></div>)}</aside>
    <section className="review-session-main">
      <div className="review-evidence-picker"><label>Evidence under review</label><select value={evidence?.id || ''} onChange={(e)=>setSelected([...(review.snapshot?.misses||[]),...(review.snapshot?.wins||[])].find(x=>x.id===e.target.value))}>{[...(review.snapshot?.misses||[]),...(review.snapshot?.wins||[])].map(x=><option key={x.id} value={x.id}>{x.title}</option>)}</select></div>
      ...
      <div className="review-trace"><h3>Execution evidence</h3><div><b>Input summary</b><span>Original goal and step-specific context</span><Link to={evidence.link || '#'}>View goal</Link></div><div><b>Output summary</b><span>{evidence.error || `${evidence.completed_steps} completed steps`}</span><Link to={evidence.link || '#'}>View output</Link></div><div><b>Retry event</b><span>{evidence.retries || 0} exception-policy retries recorded</span><Link to={evidence.link || '#'}>View events</Link></div></div></> : <p>No goal evidence is available for this period.</p>}
      <div className="review-feedback"><label>Overall rating<select value={rating} onChange={e=>setRating(e.target.value)}><option value="needs_improvement">Needs improvement</option><option value="meets_expectations">Meets expectations</option><option value="exceeds_expectations">Exceeds expectations</option></select></label><label>CEO feedback<textarea value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder="Describe the outcome expected next time…" /></label><label>Learning destination<select value={destination} onChange={e=>setDestination(e.target.value)}>{DESTINATIONS.map(d=><option key={d[0]} value={d[0]}>{d[1]}</option>)}</select></label><button disabled={!feedback.trim()} onClick={propose}>✦ Propose improvement</button></div>
    </section>
    <aside className="review-speakers"><h3>Speaking order</h3><div className="done"><b>✓ COO briefing</b><small>Evidence and outcomes prepared</small></div><div className="speaking"><b>2 Agent explanation</b><small>{evidence?.agent_id || 'Assigned agent'} presents context</small></div><div><b>3 CEO feedback</b><small>Guidance pending</small></div></aside>
  </div>;
}
...
function Improvements({ review, refresh }) {
  ...
  return <div className="review-improvement-layout"><section><div className="review-improvement-head"><h2>Proposed improvements <span>{review.improvements?.length || 0}</span></h2><mark>{review.improvements?.filter(x=>x.status==='draft').length || 0} awaiting CEO decision</mark></div>
    {(review.improvements || []).length ? review.improvements.map((item, idx)=><article className="review-improvement" key={item.id}><header><b>{idx+1}</b><h3>{item.title}</h3><mark className={item.status}>{item.status.replace('_',' ')}</mark></header><div className="improvement-body"><div><small>Preview of behavior change</small><div className="behavior-diff"><span><em>Before</em>{item.problem || 'Current behavior'}</span><i>→</i><span><em>After</em>{item.proposed_change}</span></div></div><div><small>Learning destination</small><strong>{DESTINATIONS.find(d=>d[0]===item.destination)?.[1] || item.destination}</strong><p>{DESTINATIONS.find(d=>d[0]===item.destination)?.[2]}</p><p className="soul-note">♡ Soul is not changed by this process.</p></div></div><footer><span>Owner <b>{item.owner_agent_id || 'COO'}</b></span><span>Success metric <b>{item.success_metric || 'Defined in review'}</b></span><span>Rollback <b>Versioned and reversible</b></span>{item.status==='draft'&&<><button className="secondary" onClick={()=>decide(item.id,'reject')}>Reject</button><button onClick={()=>decide(item.id,'approve')}>✓ Approve</button></>}{item.status==='approved'&&<button className="secondary" onClick={()=>decide(item.id,'rollback')}>↶ Roll back</button>}</footer></article>) : <div className="review-no-improvements"><b>No improvements proposed yet</b><span>Review evidence and add CEO feedback first.</span></div>}
    ...
    <div className="review-complete"><span>{review.status === 'completed' ? '✓ Review completed and snapshot locked' : 'Complete after every proposed improvement has a decision.'}</span>{review.status !== 'completed' && <button disabled={(review.improvements||[]).some(item=>item.status==='draft')} onClick={completeReview}>Complete review</button>}</div>
