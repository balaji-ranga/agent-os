...
export const api = {
  ...
  kanbanHumanRespond: (id, body) => post(`/kanban/tasks/${id}/human-response`, body),
  ...
  ceoWorkAssignmentPolicySave: (body) => put('/ceo-guardrails/work-assignment-policy', body),
  humanDirectory: () => get('/human-communications/directory'),
  humanChannelsSave: (userId, body) => put(`/human-communications/directory/${encodeURIComponent(userId)}/channels`, body),
  humanConversations: (params = {}) => get(`/human-communications/conversations?${new URLSearchParams(params)}`),
  humanDirectConversation: (userId) => post('/human-communications/conversations/direct', { user_id: userId }),
  humanMessages: (id, params = {}) => get(`/human-communications/conversations/${encodeURIComponent(id)}/messages?${new URLSearchParams(params)}`),
  humanMessageSend: (id, body) => post(`/human-communications/conversations/${encodeURIComponent(id)}/messages`, body),
  humanConversationRead: (id, messageId) => post(`/human-communications/conversations/${encodeURIComponent(id)}/read`, { message_id: messageId }),
  humanConversationArchive: (id, archived = true) => post(`/human-communications/conversations/${encodeURIComponent(id)}/archive`, { archived }),
  humanVoiceInvite: (targetUserId) => post('/human-communications/voice-invites', { target_user_id: targetUserId }),
  humanIncomingCalls: () => get('/human-communications/calls/incoming'),
  humanCallCreate: (body) => post('/human-communications/calls', body),
  humanCallGet: (id) => get(`/human-communications/calls/${encodeURIComponent(id)}`),
  humanCallUpdate: (id, body) => patch(`/human-communications/calls/${encodeURIComponent(id)}`, body),
  publicHumanInvite: (token) => get(`/public/human-call/${encodeURIComponent(token)}`),
  publicHumanCallCreate: (token, body) => post(`/public/human-call/${encodeURIComponent(token)}/calls`, body),
  publicHumanCallGet: (token, id) => get(`/public/human-call/${encodeURIComponent(token)}/calls/${encodeURIComponent(id)}`),
  publicHumanCallUpdate: (token, id, body) => patch(`/public/human-call/${encodeURIComponent(token)}/calls/${encodeURIComponent(id)}`, body),
  ...
  scheduledGoalsPlanPreview: (body) => post('/scheduled-goals/plan-preview', body),
  scheduledGoalsSetPlan: (id, body) => post(`/scheduled-goals/${encodeURIComponent(id)}/plan`, body),
  scheduledGoalsApprovePlan: (id) => post(`/scheduled-goals/${encodeURIComponent(id)}/plan-approve`, {}),
