...
export default function CooAssistantWidget() {
  ...
  useEffect(() => { api.agentsList().then((r) => { const rows = Array.isArray(r) ? r : r.agents || []; setCoo(rows.find((a) => Number(a.is_coo) === 1 || a.is_coo === true) || rows.find((a) => /chief operating|\bcoo\b/i.test(`${a.name || ''} ${a.role || ''}`))); }).catch(() => {}); }, []);
  if (!coo) return null;
