...
Docs: `knowledgebase/IBKR-MONTHLY-WORKFLOWS.md`, `platform-help/20-ibkr-monthly-trading.md` (Summary UI + clear; **ingest URL vs W3 EOD run**), Phase 4 runbook `IBKR-MONTHLY-PHASE4.md`. UI route: `/ibkr-summary`. Default laptop `WEBHOOK_URL` is `/api/ibkr-trading/local-bridge-webhook` (W3 secret); 5‑min snapshots do not start W3.

W1 hard gates + W2 `/execute-day-plan` refuse BUY limits more than `entry_discount_pct_max` (default 3%) below last (or with no quote). W1 `risk_per_trade_pct` (default **5**) is the max stop % below entry per order; blank/0 lets the Maker choose. W1 requires a **bookable new_entry**: either a full bracket (`qty`, `entry_price`, stop below, tp above) **or** hold-for-weeks (`bracket` false, `later_day_plan`, `forecast_up_weeks` ≥ 1, omit tp). Screener enrich (FMP PE / SMA / momentum / YoY on `screener_enrich_limit` names, default 25 matching `screener_limit`) feeds grind vs swing; Brave Search is the fallback when those fields are missing. New-entry notional sizes to min(daily budget, cash, `position_size_pct_max` of equity). Direct `/place-bracket` smoke scripts can still use intentionally cheap limits. Company setup deep pack `demo_balaji_ranganathan` embeds the same W1 bindings + band variables; after editing the frozen JSON run `node scripts/patch-demo-blueprint-ibkr-quote-band.js` then `FROM_PACK_FILE=1 node scripts/publish-balaji-demo-blueprint.js` so published DB rows match the pack. Refresh golden IBKR templates: `node scripts/export-standard-ibkr-workflows.js` (writes `company-blueprints/standard/trading/workflow-w*.json`).
