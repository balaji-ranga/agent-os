...
| Blueprint snapshot / publish | `backend/src/services/company-blueprint-publish.js`, `company-blueprints/registry.js`, `company-blueprints/secret-sanitize.js` (zip v2: knowledge, policies, org, agents+tools+ops MD, workflow graphs, connectors; **live secrets scrubbed** on Admin publish/export; vault `*Ref` kept; `scripts/validate-company-blueprint-export.mjs`) |
