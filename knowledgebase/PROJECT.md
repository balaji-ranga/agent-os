...
3. After login you land on **home chat** (`/`) with the **COO** selected by default. A compact **How your company runs** strip sits above the dashboard. Switch agents from the **Chat with** picker. **History** and **Browser session** side panes start closed — use the icons next to **New chat** to open them. **New chat** archives the current thread; on local Ollama the archive title is heuristic so **Archiving…** does not wait on the same CPU model. Org chart, architecture diagrams, and standups are under **My Org** (`/org`). Public architecture: [https://flolah.cloud/docs/start/how-the-company-runs/](https://flolah.cloud/docs/start/how-the-company-runs/).
...
6. Optional **Efficiency mode** (Yes/No, default No): **Yes** sends learnings summary, chat archive titles, Brain / IBKR recaps, broadcast/COO classify, leftover goal-plan args, and policy/goal enrich to local Ollama instead of `Platform_BYOK`. Agent Chat stays on the Profile provider.
7. Optional **per-tool model**: **Tools** → **Tools → Model** overrides Profile (or platform) model for specific BYOK-aware tools only (keys still from Profile/vault). Efficiency mode **Yes** ignores that mapping for the wave-1 jobs above. Optional **Tools → Rate limits** sets per-user daily/monthly **call** caps for API-key tools (on top of per-agent token budgets). Help: `knowledgebase/platform-help/11-content-tools-scripts-profile.md`.
8. Browse uploads and generated media under **Content Explorer** (`/content-explorer`).
9. Optional: avatar → **Company setup** (`/company-setup`) for the structured company wizard (help **29**), and/or **Onboarding** / **Onboarding Helper** for freeform dept/agent proposals (help **27**).
10. Optional: **Management → Scheduled goals** or COO chat for lasting daily/weekly prompts (help **28**). Constrained pipeline outcomes: help **48**.
11. Optional (content creator): **Connectors → MCPs** for Facebook Page OAuth (help **31** setup; optional CEO App ID/secret override); publish / community / weekly ops rollup (help **30**). Ops rollup uses the **bell** (`notify_ceo`), not email.
12. Keep MFA settings as required by your organization.
13. Set **Data persistence** (30 / 60 / 90 / 120 / 365 days). A nightly job permanently deletes your chats, chat history, standup conversations and workflow runs older than that; **Purge aged data now** does it immediately. Watch the effect on **Efficiency View → Org → Storage (MB)**.
...
| **Example stress test run** | COO outcomes: Gate A 40 verified / $75; Gate B live research; Gate C CRM path + approval; Gate D 30-day not run. Help **48**. Public: `/docs/operate/example-stress-test-run`. |
...
| **API Keys vault** | Settings → API Keys — named secrets, optional encryption. Non-platform Profiles auto-seed unset slots (`Platform_BYOK`, `Replicate_BYOK`, `BRAVE_SEARCH_BYOK`, `GOOGLE_PLACES_BYOK`, `X_API_BYOK`, `INSTAGRAM_SESSIONID`, `elevenlabs-key`). Chat, vision, image gen, and live Call share **`Platform_BYOK`** (OpenAI / OpenRouter; Claude via OpenRouter; live Call needs an OpenAI key). Tool vs key inventory + SME vendor-cost ranges: help **15**, public [API keys](https://flolah.cloud/docs/setup/api-keys/). Platform default Places uses `GOOGLE_PLACES_API_KEY`, or vault **`GOOGLE_PLACES_BYOK`** if the platform key is unset. Optional **Tools → Model** overrides model per content tool without changing vault keys. Optional **Tools → Rate limits** caps vendor API **calls** per CEO (daily/monthly). |
...
| **Efficiency View** | **Org** tab: agents, automated tasks, feedback, workflow run success/fail, Storage (MB) (7d–All). **Department** tab: month-to-date tokens vs each department's budget **plus people task counts**. **Agent View** tab: per-agent activity, outcomes, token/error budget gauges, **Reset usage**. **User View** tab: Kanban performance for people (`/efficiency?tab=user`). **LLMOps** tab: tokens, estimated $, traces, price book (`/efficiency?tab=llmops`). |
...
| **Platform logging & redaction** | `PLATFORM_LOG_LEVEL=off\|error\|info` for backend request/error logs. Secrets (API keys, bearer tokens, `Authorization`, passwords, MFA codes) are redacted from URLs, JSON bodies and headers; API Keys / auth routes log method + route only. Unit tests: `backend/scripts/test-security-hardening-unit.js`, `backend/scripts/test-ssrf-outbound.js`, `backend/scripts/test-blueprint-secret-sanitize.js`, `backend/scripts/scan-blueprint-secrets.js`. GitHub CodeQL JS: re-enable JavaScript in **default setup** (advanced workflow cannot upload while default setup is on). |
...
| **Home chat** | `/` — COO chat by default; agent picker; history/browser panes; compose **microphone** (speak, pause 3s after you finish, auto-send) / **Speak** (full reply in Piper chunks) / **phone** (Call when Channels → Voice is enabled on that employee, COO included); **OEI** KPI + explain popover (help **36** / **25** / **46**). On phones: overview first, **Chat** opens a full-screen sheet; history/browser icons open a side drawer (desktop unchanged). |
...
| **Workflow Builder chat** | Natural-language builder: **create/build asks create a graph and open the editor**; **edit asks** (add a connector/tool/agent/MCP) look up catalogs and **mutate the open graph**. Curated recipes when they match; otherwise the model picks catalog nodes. Public upload/publish waits for CEO approval. |
...
| **Digest (This Week)** | Top-nav `/this-week`: Time Saved, Est. Value Delivered (sum of completed work x each AI employee `hourly_rate_usd`, hire default $10/hr), **2 most recent durable goal plans** (`agr-…`) for the week (**Execution trace** + **View all** → `/goal-plans` with optional week `start`/`end`), insights. Env: `THIS_WEEK_MINUTES_PER_TASK`, fallback `THIS_WEEK_VALUE_USD_PER_HOUR`. COO tool `this_week_digest`. Platform Help **02** / **28**. |
...
| **Profile LLM catalog** | Provider + model picker on **Register** and **Profile** (`llm_provider` / `llm_model` / `llm_efficiency_mode`); `GET /api/auth/llm-catalog`; AgentSystem sync on Profile save. **Efficiency mode** routes wave-1 utility jobs to local Ollama. BYOK keys only via API Keys vault after login. |
...
- `POST /tools/summarize-url` — HTTPS **public** pages only (SSRF: no loopback/private/link-local/metadata; redirects re-checked). Optional `TOOLS_SUMMARIZE_ALLOWED_DOMAINS`.
...
- Ledger `token_usage` sources: `openclaw_chat`, `delegation`, `workflow_brain`, `a2a_outbound`, `content_tool`, `goal_planner`, `chat_completions`; provider usage when returned, otherwise a flagged `chars/4` estimate; optional `trace_id`
- `GET /efficiency/llmops?days=` — LLMOps tab (tokens, estimated $, traces, quality)
- `GET/PUT /efficiency/price-book` — CEO token price book (USD / 1M); platform catalog not mutated
- `GET/POST /efficiency/cost-lines`, `DELETE /efficiency/cost-lines/:id` — manual outside costs (owner-scoped)
- COO tool `POST /tools/llmops-summary`
...
│       ├── db/
│       ├── lib/                # content-tools-allow; outbound SSRF guard (ssrf.js)
