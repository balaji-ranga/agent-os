...
| **Home chat** | `/` — COO chat by default; agent picker; history/browser panes; compose **microphone** (speak, pause 3s after you finish, auto-send) / **Speak** / **phone** (Realtime Caller Voice channel only); **OEI** KPI + explain popover (help **36** / **25** / **46**). On phones: overview first, **Chat** opens a full-screen sheet; history/browser icons open a side drawer (desktop unchanged). |
