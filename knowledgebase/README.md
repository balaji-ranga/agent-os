...
| **platform-help/** | CEO end-user Platform Help corpus (RAG). Highlights: **02** Digest/Workspace + **global search (task/run ids)** /Builder/nav/OEI; **03** specialty-first COO; **04** Kanban (Weekly default, agent filter, Select all, task ID); **06** WF run id; **11** Tools/Model/media; **12** Workspace vs Kanban + queued Agent Chat; **16** OpenConnector BYOA App override + OAuth/API key; **17** desktop; **19–20** crons + IBKR; **21–27** external→onboarding; **28–32** goals/setup/content/MCP OAuth/Business Core; **33–40** IP/tokens/company details/OEI/P&L design/Maker-Checker/Tier A ERP/CRM; **41** video studio (Flow S4 serial); **42** Social Researcher + Business Discovery (Places + vault **GOOGLE_PLACES_BYOK**); **43** Admin AgentSystem recovery (OTP privileged session); Platform Help **answer-first** |
