...
Proposed live evolution, including the user-controlled Paper/Live mode, shared tested configuration, mode-isolated execution data and Gateway account attestation, is documented separately in [IBKRNEW-LIVE-TRADING-IMPLEMENTATION-PLAN.md](./IBKRNEW-LIVE-TRADING-IMPLEMENTATION-PLAN.md). It is not part of the implemented paper release.
...
- `owner_user_id`
- `account_ref` (a random, bridge-scoped `IBKRNewAccount_*` value generated by Flolah)
- `bridge_id`
- `environment` (`paper`; `live` reserved for future use)

The real IBKR account identifier exists only in the desktop bridge's local environment and is supplied directly to IB Gateway/TWS. It is never entered in the browser, sent to Flolah, stored in the VPS database, embedded in commands, or returned by Flolah APIs. Server persistence uses only `account_ref`; ingress recursively removes account-number fields and redacts recognizable IBKR account values in free text before any raw or projected write.
...
Commands are signed or MAC-protected over their canonical contents. The desktop verifies signature, bridge, owner, opaque account reference, environment, policy version, strategy version, expiry, and nonce before execution, then independently uses the locally configured paper account for IBKR submission.
...
```json
{
  ...
  "account_ref": "IBKRNewAccount_uuid",
  "bridge_id": "bridge",
  "environment": "paper",
...
- Keep real IBKR account identifiers desktop-only; persist and display only random bridge-scoped account references.
- Strip account-number keys and redact recognizable IBKR account values from every event, snapshot, health record, and broker error before persistence, including owner-visible records.
- On upgrade, replace legacy account identifiers in all IBKRNew tables and JSON/text projections, cancel pending/claimed commands whose signatures would be invalidated, securely checkpoint and rebuild SQLite storage, and rotate any historical backups that contain the legacy values.
- The account-reference cutover revokes legacy bridge credentials, requires desktop protocol version 2 for command claims, and rechecks the current account reference through bootstrap immediately before broker submission. Operations must stop pre-upgrade desktop runtimes and allow already-issued short-lived authorizations to expire before starting upgraded credentials.
