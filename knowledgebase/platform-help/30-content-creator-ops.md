...
**How do I run ops rollup now?** **Scheduled goals → Run now** on “Weekly ops rollup for CEO”, chat Ops Reporter, or chat/workflow phrase: **run ops rollup**.
...
## Workflows & chat phrases (typical)
...
1. Admin/operator has set Meta **App ID** + **App Secret** on the platform (`FACEBOOK_APP_ID` / `FACEBOOK_APP_SECRET` or Connectors MCPs admin config). Development-mode apps work if your Facebook user is Admin/Tester on the app.
...
See also: [16-connectors-openconnector.md](./16-connectors-openconnector.md) (MCPs tab), [08-mcp-integrations.md](./08-mcp-integrations.md).

## LinkedIn & other channels

- **LinkedIn** via **OpenConnector** when that connection is active under Connectors → OpenConnector.
...
- **Kanban → awaiting you** for draft content and sensitive community replies.
- Approve with a clear decision; production graphs may pass an **approved content summary** into the publisher leg (not only the word “approved”).
...
- Connectors / MCPs: [16-connectors-openconnector.md](./16-connectors-openconnector.md)
- MCP registry detail: [08-mcp-integrations.md](./08-mcp-integrations.md)
...
- Company setup pack: [29-company-setup.md](./29-company-setup.md)
