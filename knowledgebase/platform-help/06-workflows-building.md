...
The designer chat uses **your Profile language model**: **BYOK** (`Platform_BYOK` + the provider/model you picked) or **platform primary** when Profile is Platform default. Optional **Tools → Model** override: `workflow_builder_chat`. It does not send a DeepSeek model id to OpenAI (that yields “model does not exist”).
...
Talk in **plain English** (“look this up and send me a recap”, “promote this product on Medium and Hacker News with blogs”, “generate story scenes, review them, and upload to YouTube”, “scrape IMDb and Rotten Tomatoes for reviews”). You do not need node names, curl, or JSON. **Any create/build-a-workflow ask produces a real graph and opens the editor**. In the open editor, say **add a GitHub connector**, **wire a content tool**, or **fix this node** — the builder searches Connectors / content tools / employees / MCP (exact ids) and **updates the graph**, it does not call those catalogs as failed chat actions. Workflow Builder gives the model the node catalog (purpose, inputs, outputs, config), this company’s **employees** (id + role), **content tools**, **MCP**, and **connectors**, and the model **chooses the nodes**. It does not pick employees by matching a word in your ask (so “reviews” does not become ERP/CRM Checker). Public upload/publish waits for **your approval**. After a build, it lists any **Settings → API Keys** names to store. Secrets must never be pasted into chat or node fields — bind a key name instead. Brain uses **free Ollama** and picks a model that is already installed (no key). It does not copy the platform’s paid/cloud model name into the graph. If Ollama is down, the graph binds `Platform_BYOK` and tells you to fill it. Medium uses vault name `MEDIUM_INTEGRATION_TOKEN`; Hacker News uses **Connectors** (no secret in the graph). Sites without a Connector (for example YouTube) use **Browser Session** (`browse_task_start`) after you are signed in.
