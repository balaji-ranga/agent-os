...
CEO-facing inventory (chat / vision / live Call grouped, plus SME cost ranges): [15-api-keys-vault.md](./15-api-keys-vault.md) and public [API keys](https://flolah.cloud/docs/setup/api-keys/).
...
| **Ollama (local)** | Free / local BYOK chat; **optional platform + AgentSystem primary** (`PLATFORM_USE_LOCAL_OLLAMA=1`); Brain `ollama`; **Profile Efficiency mode** (wave-1 utility jobs) | Usually no real key (`ollama` / `OLLAMA_API_KEY` placeholder) | Local Ollama service + Profile / Brain / ops `.env` | Optional (required if platform primary is local, or Efficiency mode is Yes) | Not a paid vendor API; never Ollama Cloud. Wanted model `mistral-medium-3.5` (128B) needs ~80GB GPU/RAM. CPU 16GB VPS uses `llama3.2` — 8B at 65k KV was OOM-killed (408 timeout). Catalog `OLLAMA_CONTEXT_WINDOW=65536`, runtime `OLLAMA_NUM_CTX=32768`. **New chat** uses a heuristic archive title when the **whole** platform primary is local Ollama so **Archiving…** does not block the CPU model. Profile **Efficiency mode = Yes** still titles via Ollama when the platform primary is paid. See operator notes for local Ollama. |
| **Brevo (SMTP)** | MFA email OTP, workflow **Send Email**, `email_send` tool (ICS invites), COO status HTML email | `WORKFLOW_SMTP_HOST` / `USER` / `PASS` / `FROM` (Brevo: `smtp-relay.brevo.com`) | Platform `.env` (or per-node SMTP) | **Yes** for outbound email / email MFA | Sender domain must be verified in Brevo. In-app `notify_ceo` does **not** need SMTP. **Never** put SMTP user/password on a published company blueprint — Send Email nodes use `useEnvSmtp` and empty `smtpUser` / `smtpPass`. |
...
| **Instagram (Instaloader sidecar)** | `social_research_instagram` captions/timestamps | Vault **`INSTAGRAM_SESSIONID`** (cookie); optional platform `INSTAGRAM_SESSIONID` | **API Keys** (preferred) | Optional | Sidecar is **self-hosted**. Anonymous instagram.com calls from a VPS IP return **HTTP 429** (not 409 / not Instaconnect SaaS). Without the cookie, post **images** still hydrate from `/p/{shortcode}/media`. |
...
| **Financial Modeling Prep (FMP)** | Market regime, screener, history, fundamentals (IBKR monthly trading tools) | `MARKET_DATA_API_KEY` | Platform `.env` | Optional (required for live market tools) | Default base `https://financialmodelingprep.com/stable`. Free tier: 250 calls/day. Regime uses EOD light + skip 402/invalid tickers + `MARKET_DATA_REGIME_FALLBACK_SYMBOLS`. `company-screener` is paid (universe fallback). Paid recommended for large daily screens. |
...
| **OpenConnector SaaS apps** | Workflow **Connector** node (~**1,300** catalog connectors: Gmail, Slack, HubSpot, GitHub, …) | OAuth token or vendor API key per app | **Connectors** UI + optional **API Keys** vault | Per app | Catalog from [Open Connector](https://github.com/oomol-lab/open-connector) / [openconnector.dev/#connectors](https://openconnector.dev/#connectors). Each connected app is its own external dependency. |
...
| Local free | Ollama (`optional-ollama`) | No paid key; local Ollama service |
