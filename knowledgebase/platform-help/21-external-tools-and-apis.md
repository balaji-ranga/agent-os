...
| **OpenAI** | Secondary / BYOK chat; image content tool (`TOOLS_IMAGE_*`); **analyze_image** vision (platform primary or BYOK `Platform_BYOK`); optional OpenSearch embeddings | `OPENAI_SECONDARY_API_KEY` or vault **`Platform_BYOK`**; embeddings reuse OpenAI-compatible key when enabled | Platform `.env` and/or **API Keys** | Optional | Profile → OpenAI preference needs `Platform_BYOK`. Image gen needs a key that can call GPT-image (or compatible host). Vision uses platform **primary** for Platform default Profiles (fails if primary is text-only); BYOK Profiles use vault only. |
