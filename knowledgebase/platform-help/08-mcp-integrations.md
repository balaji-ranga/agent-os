...
- **Social Research MCP:** HTTP service `social-research-mcp` (`optional-social-research-mcp`, port 8084) plus Instaloader sidecar (`:8083`). Same tools as `social_research_*` / `business_discover` (Instagram/X **posts[]** hydration). Pass **`X-Ceo-User-Id`**. Hire **Social Researcher** / **Business Discovery** from AgentExchange. Help **42**.
