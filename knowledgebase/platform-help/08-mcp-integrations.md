...
- **Web Scrape MCP:** HTTP service `web-scrape-mcp` (`optional-web-scrape-mcp`, port 8085). Crawlee Cheerio + Playwright. Tools `scrape_url` / `scrape_domain`. Workflow palette **Web Scrape**. Pass **`X-Ceo-User-Id`**. Help **[44](./44-web-scrape.md)**.
