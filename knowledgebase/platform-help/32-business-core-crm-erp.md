...
7. **Deleted CRM desks + Twenty 2.29 (fixed):** Reclaimed/soft-deleted Twenty desks used to leave `userWorkspace` rows. Twenty 2.29 then crashes in CheckUserExists / GetCurrentUser (`workspaceDiscoverability` on a null workspace) — SSO apply can succeed and the iframe still shows `/welcome`, and typing email shows **An error occurred while checking user existence**. CRM open now detaches those stale memberships before minting; reclaiming a desk also detaches them. Do not type a password into Twenty.
