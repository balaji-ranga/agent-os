...
5. Browser handoff on the **workspace origin** `https://{subdomain}.crm.<apex>/flolah-handoff/?owner=…&wipe=1&next=/verify&lt=…` (login JWT is its own `lt` param / hash, not nested as `next=/verify?loginToken=` — browsers can split that nested `?` and Twenty then opens **email login** `/welcome`). After a storage wipe, the handoff page sends the iframe to Twenty’s own `/verify?loginToken=…`. That SPA page exchanges the LOGIN token (in-memory + localStorage). Do **not** send the iframe to `/` after a separate apply HTML — that hop skips `/verify` and Chrome often drops third-party iframe storage. `/flolah-crm-sso` remains as a fallback apply URL. If you still see Twenty email login after a CRM reload, use **Open in new tab** (first-party) and do not type a password into that form.
