...
3. **History** and **Browser session** side panes are **hidden by default**. Use the clock (history) and window (browser session) icons next to **New chat** to open or close them. History has two views: **Assigned work** shows the selected employee's company-scoped Kanban/delegation work, result, isolated execution transcript, task activity, and an **Open in Kanban** link; **Conversations** shows manually archived chats and voice transcripts. **Continue with employee** copies the retained result into the active chat composer without merging the isolated execution session into the live chat. The work list is server-paginated and bounded by the company's Profile retention window. **New chat** archives the current thread (title from the first user message). On local Ollama, that title is heuristic — the button should not stay on **Archiving…**.
...
7. Optional **microphone** icon (Whisper: click, speak, pause **3 seconds after you finish**, auto-send) and **Speak** (Piper plays the **full** reply in chunks) when free speech is deployed — [25-speech-and-published-scenes.md](./25-speech-and-published-scenes.md). The **phone** icon is **Call** when **that** employee has **Channels → Voice** enabled (COO included; Realtime Caller is the dedicated hire) — [46-voice-caller-employees.md](./46-voice-caller-employees.md). Slow Caller without Voice has no Call.
