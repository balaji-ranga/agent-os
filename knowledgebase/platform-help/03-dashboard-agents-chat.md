...
7. Optional **microphone** icon in the compose box (Whisper) and **Speak** (Piper) when free speech is deployed — [25-speech-and-published-scenes.md](./25-speech-and-published-scenes.md). The **phone** icon starts a live WebRTC session (OpenAI Realtime BYOK) — [46-voice-caller-employees.md](./46-voice-caller-employees.md). Same toolbar on Home (`/`) after you **Chat with** Slow Caller or Realtime Caller.
