...
7. Optional **microphone** icon (Whisper: click, speak, pause **3 seconds**, auto-send) and **Speak** (Piper) when free speech is deployed — [25-speech-and-published-scenes.md](./25-speech-and-published-scenes.md). The **phone** icon is **Call** only when that employee has an enabled Voice channel (Realtime Caller) — [46-voice-caller-employees.md](./46-voice-caller-employees.md). Slow Caller has no Call.
