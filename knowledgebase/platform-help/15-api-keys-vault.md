...
**Workflow Builder:** when you describe a flow in plain English, the builder **creates a graph** (not a chat-only plan) and **never stores the secret in the graph**. It binds a key **name** (often `Platform_BYOK`) and ends with a short list of names to add here. Free Ollama Brain steps need no key. Medium publishing binds **`MEDIUM_INTEGRATION_TOKEN`**. Hacker News posting uses **Connectors**. Video generate may need **`Replicate_BYOK`**. YouTube/TikTok-style upload uses **Browser Session**, not a vault YouTube key.
