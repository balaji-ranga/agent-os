...
**agent_tool args vs chat (important):** Free-form **chat** fills tool parameters inside the AgentSystem agent loop (same agent model as your COO chat). **Goal-plan data `agent_tool` steps** (market_*, status_checker, list workflows, …) resolve args like chat: (1) heuristics expand baskets (MAG7 / MAGS / Magnificent 7) and tickers; (2) owner-aware LLM fill when still sparse; (3) multi-ticker tools invoked **once per symbol**. **Agent interpretation (generic):** compositional outbound tools such as **`email_send`** that follow data/workflow/specialty steps are rewritten to **`agent_continue`**. When a prior tool already produced **HTML/markdown** (status digest, week digest, …), the **platform sends that artifact once** via `email_send` (correct `html=` + markdown body) and the agent must **not** invent a short plain-text substitute or call `email_send` again. Chat-synthesis goals still run AgentSystem after delivery for a brief confirm. Prompts that say **do not call notify_ceo** skip the notify step.

**Session / memory on scheduled + goal-plan fires:** Standup/delegation prompts historically wrapped AgentSystem turns with “read MEMORY.md / sessions_history; if you already did this today, ask whether to redo.” That **dedupe hint confuses scheduled goals** (and goal-plan `agent_continue`): the COO may reuse an earlier digest narrative or invent a short email instead of executing this fire’s tools. Scheduled + goal-plan paths now use a **fresh-run** wrapper and disable session-history injection. Standup/delegation still use the memory dedupe helper.

**Which LLM?** Plan **intent classification** and **tool-arg fill** use backend `chatCompletions` (`goal_plan_intent` / `goal_plan_tool_args`) — owner BYOK or platform default, optional Tools-menu model override. **`agent_continue` / specialty** use the **AgentSystem agent model** when a free-form turn is still needed. Dry data `agent_tool` HTTP invoke does **not** use the chat tool-loop once args are resolved.

**Failure recovery Kanban:** When a goal plan reaches **failed**, platform creates **one** recovery Kanban + pending delegation for the owning agent (specialty agent if that step failed, else COO). The card body is wrapped `[SYSTEM goal_plan_recovery]` with the **verbatim CEO goal** and a **step ladder** (completed / failed / pending + errors). Rules: **do not** call `agent_goal_create` (no new `agr-…`); finish via normal **Agent Chat tools**. Disable with `GOAL_PLAN_FAILURE_KANBAN=0` in `deploy/.env` (**also wired through `deploy/docker-compose.yml` backend-env** — a `.env`-only change without recreate does nothing), **or** Admin → **AgentSystem recovery** kill-switch (platform setting, no recreate). Idempotent via `context_json.failure_recovery_kanban_at`. The Kanban orphan watcher **does not reinitiate** recovery cards when the kill switch is off, or when the failure is permanent config (e.g. Google Places not configured) — that avoids AgentSystem saturation so COO chat can still respond. CEO also still gets the terminal completion chat nudge.

**Specialty vs workflow steps:** Workflow maker-checker phrases become `workflow_trigger` steps (phrases survive plan storage—renormalizing step specs is idempotent). Remaining multi-intent work (research, design, copy, Platform Help how-to, etc.—not limited to two) becomes `specialty_task` steps. **Lettered `A) … B) …` (or `1) … 2) …`) is sequential** (not parallel). Unlettered multi-specialist residual may still run in parallel. Explicit “via Platform Help …” is always a specialty step. Plan leftovers (`notify_ceo`, `agent_goal_create`) are stripped before specialty classification. **Kanban / Master Data / notify words inside a specialist’s instructions do not drop all specialty steps** (that used to collapse the draft to **Schedule agent continue**). COO-native skip applies only when the residual is coordination-only (no lettered list, no named org employee). Hybrid prompts keep both (workflow phrases plus residual specialty).
...
2. **Specialty intents (LLM)** — After those phrases are removed, the **residual** text is classified with the CEO’s `AGENTS.md` via intent allocation (LLM). That produces `specialty_task` steps (research, design, Platform Help how-to, …). Lettered lists like `A) … B) …` are **split and sequential** (including when the prompt is one paragraph — `A)` after a space still counts); plain “and / also” stays one residual for the model. If the model returns empty JSON, lettered chunks that **name an org employee** still become specialty steps (live under-COO roster, not only AGENTS.md). A preamble before `A)` is not a specialist. Kanban / Master Data / notify words in those instructions do not become COO `kanban_create_task` steps or collapse the plan to Schedule-agent continue. Notify stays last.  
...
| **Scheduled goal fire (chat fallback)** | Isolated per fire | Each fire uses a unique `sched-{goal}-{run}` AgentSystem session (not shared with other schedules on the same agent). Prefer approve-plan schedules for multiphase CRM/ERP. |
...
- Pipeline example (outcome KPI, $75 cap, Gate A): [48-pipeline-under-constraints.md](./48-pipeline-under-constraints.md)
