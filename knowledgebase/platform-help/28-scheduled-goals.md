...
**agent_tool args vs chat (important):** Free-form **chat** fills tool parameters inside the OpenClaw agent loop (same agent model as your COO chat). **Goal-plan data `agent_tool` steps** (market_*, status_checker, list workflows, …) resolve args like chat: (1) heuristics expand baskets (MAG7 / MAGS / Magnificent 7) and tickers; (2) owner-aware LLM fill when still sparse; (3) multi-ticker tools invoked **once per symbol**. **Agent interpretation (generic):** compositional outbound tools such as **`email_send`** that follow data/workflow/specialty steps are **not** dry-invoked with a goal-plan dump. The planner **rewrites** them into **`agent_continue`** so the COO OpenClaw turn interprets prior outputs (including digest HTML/markdown) and calls tools like chat. If an older saved plan still has a compositional `agent_tool` after priors, execute routes that step through the same OpenClaw interpretation path. Chat-synthesis / HTML-email / “craft report” goals also append `agent_continue` when needed. Prompts that say **do not call notify_ceo** skip the notify step. Digest HTML wiring remains a dry-path safety net only.

**Which LLM?** Plan **intent classification** and **tool-arg fill** use backend `chatCompletions` (`goal_plan_intent` / `goal_plan_tool_args`) — owner BYOK or platform default, optional Tools-menu model override. **`agent_continue` / interpreted compositional tools / specialty** use the **OpenClaw agent model** (same as Agent Chat). Dry data `agent_tool` HTTP invoke does **not** use the chat tool-loop once args are resolved.
