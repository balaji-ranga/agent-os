...
2. **Specialty intents (LLM)** — After those phrases are removed, the **residual** text is classified with the CEO’s `AGENTS.md` via intent allocation (LLM). That produces `specialty_task` steps (research, design, Platform Help how-to, …). Lettered lists like `A) … B) …` are **split and sequential**; plain “and / also” stays one residual for the model. If the model returns empty JSON, lettered chunks that **name an org employee** still become specialty steps (live under-COO roster, not only AGENTS.md). Kanban / Master Data / notify words in those instructions do not become COO `kanban_create_task` steps or collapse the plan to Schedule-agent continue. Notify stays last.  
