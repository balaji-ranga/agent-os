...
**agent_tool args vs chat (important):** Free-form **chat** fills tool parameters inside the OpenClaw agent loop (same agent model as your COO chat). **Goal-plan `agent_tool` steps** used to call tools with stored `spec.args` only — often `{}` — so tools like `market_history` failed with “symbol is required” even when the goal text listed MAG7/tickers. **Execute path now** resolves args like chat: (1) heuristics expand baskets (MAG7 / MAGS / Magnificent 7) and tickers from parenthetical or `$TICKER` lists; (2) owner-aware LLM fill (`goal_plan_tool_args` via Profile BYOK / platform LLM) when still sparse; (3) multi-ticker tools that take a single `symbol` are invoked **once per symbol** and aggregated. After data tools, if the goal asks for a **summary / report / reply in this chat**, the planner appends **`agent_continue`** so the COO synthesizes in chat the same way as a normal prompt. **Status digest email:** when a prior step is `status_checker` (or `this_week_digest`), `email_send` uses that step’s **HTML/markdown digest** — not a raw goal-plan dump. Prompts that say **do not call notify_ceo** skip the notify step.

**Which LLM?** Plan **intent classification** and **tool-arg fill** use backend `chatCompletions` (`goal_plan_intent` / `goal_plan_tool_args`) — owner BYOK or platform default, optional Tools-menu model override. **`agent_continue` / specialty** use the **OpenClaw agent model** (same as Agent Chat for that AI employee). Dry `agent_tool` HTTP invoke does **not** use the chat tool-loop for each call once args are resolved.
