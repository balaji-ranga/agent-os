...
- **Full bracket** (`bracket: true`): qty, entry, stop below, take-profit above. W2 places parent BUY + stop + TP. `forecast_up_weeks` here is only a thesis horizon — it does not turn the order into hold-for-weeks.
