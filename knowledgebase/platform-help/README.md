...
| [28-scheduled-goals.md](./28-scheduled-goals.md) | **Scheduled goals** — draft/approve multi-intent plans; **Amend plan manually** (intent→step baseline); `agent_goal_*` **async ack** + once-only completion chat nudge; data `agent_tool` arg fill (MAG7/tickers); compositional tools (`email_send`) → **`agent_continue` interpretation**; schedule fire → new `agr-…`; terminal notifies name plan; e2e `test:e2e:goal-plan` / `test:goal-plan:async-ui`; Digest 2 + `/goal-plans` |
...
| [42-social-research-business-discovery.md](./42-social-research-business-discovery.md) | **Social Researcher** + **Business Discovery** (Discover → Research → Track → Act via existing goal plan + tools; Social Research MCP; Places API (New) + vault **GOOGLE_PLACES_BYOK**; Instaloader + **INSTAGRAM_SESSIONID**; X hydrate / **X_API_BYOK**; Knowledge `discovered_opportunities`; CRM Kanban only on Act) |
