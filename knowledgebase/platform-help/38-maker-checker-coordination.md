...
**Plan intent (catalog + LLM):** published workflow phrases → specialty residual (AGENTS.md) → orchestrator tools (`notify_ceo`, `agent_workflow_list`, `email_send`, …). Create-goal / compliance lines are skip. Acceptance: `test-goal-plan-intent-classify.mjs`.
