...
**Plan intent (catalog + LLM):** published workflow phrases → specialty residual (AGENTS.md) → orchestrator tools (`notify_ceo`, `agent_workflow_list`, `email_send`, `market_history`, …). Create-goal / compliance lines are skip. **`agent_tool` execute** fills missing args from the goal text (MAG7→tickers, list symbols; multi-call when needed) and may append **`agent_continue`** for chat-style synthesis. Acceptance: `test-goal-plan-intent-classify.mjs`, `test-goal-plan-tool-args.mjs`.
