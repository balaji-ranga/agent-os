...
**Completion chat nudge (once):** on plan terminal (`completed`/`failed`), platform posts **one** COO chat status (step ladder + `agr-…`) and a correlated CEO notification so progress is not only in the Goal Plan panel / step result JSON. Deduped with `coo_completion_nudge_at`. Disable with `GOAL_PLAN_COO_COMPLETION_NUDGE=0`.
...
**Plan intent (catalog + LLM):** published workflow phrases → specialty residual (AGENTS.md) → orchestrator tools (`notify_ceo`, `agent_workflow_list`, `email_send`, `market_history`, …). Create-goal / compliance lines are skip. **Data `agent_tool` execute** fills missing args from the goal text (MAG7→tickers, list symbols; multi-call when needed). **Compositional tools** (`email_send`, …) after prior work are rewritten to **`agent_continue`** (agent interpretation like chat) so emails/reports are composed from prior outputs—not atomic plan dumps. **On plan failure**, once-only **recovery Kanban** (`goal_plan_recovery`) runs chat tools—not a new goal plan. Acceptance: `test-goal-plan-intent-classify.mjs`, `test-goal-plan-tool-args.mjs`, `test-goal-plan-specialty-coo-native.mjs`.

**Step kinds:** `workflow_trigger` / `specialty_task` / `agent_tool` (orchestrator tools) / `notify_ceo` — planned via intent classify. Legacy note: `workflow_trigger` (published WF by chat phrase—must keep the exact phrase when the plan is saved), `specialty_task` (residual multi-intent / Platform Help / research), `agent_continue` (wake the goal’s owning agent once), `notify_ceo`. Specialty makers/checkers for CRM/ERP run **inside** child workflows; plan-level `specialty_task` is for residual specialists outside those graphs.

**Inspect a plan:** **Run & Operate → Goal plans** (`/goal-plans`) or Digest. Click **Execution trace** (`/goal-plans/agr-…`) for KPI, plan version history, and telemetry. CEO API `GET /api/agent-goal-runs` and `GET /api/agent-goal-runs/:id` (login, owner-scoped); events at `GET /api/agent-goal-runs/:id/events`. COO tools **`agent_goal_list`** / **`agent_goal_status`**. **Chat** shows a live Goal Plan panel when the assistant mentions `agr-…` or tools `agent_goal_*` (same **Execution trace** link). **Scheduled goals → Last plan** expands recent fires. **Digest** shows the **2** most recent goal plans for the selected week; **View all plans** lists every plan in that week (`/goal-plans?offset=`).
...
Tools are granted to COO/Workflow Builder allowlists (and to any specialty employee granted `agent_goal_*`) and registered in the AgentSystem content-tools plugin contracts (`agent_goal_*`).
...
- SME docs (specialist `master_data_rag` + Platform Help): [39-erpnext-help-tier-a.md](./39-erpnext-help-tier-a.md), [40-twenty-crm-help-tier-a.md](./40-twenty-crm-help-tier-a.md)

## Multi-specialty hybrid goal plans

In addition to CRM/ERP maker-checker workflow steps, residual speciality work in the same goal (research, design, copy, multi-step same agent, **more than two intents**) becomes `specialty_task` steps. Hybrid goals keep workflows **and** speciality residue. Parallel specialty steps share a `parallel_group` and advance when all group members finish. **Scheduled goals → Generate draft plan** is **COO-only**; other employees **Save & schedule** without a specialty ladder (help **28**). COO tools auto-approve. See [28-scheduled-goals.md](./28-scheduled-goals.md).
