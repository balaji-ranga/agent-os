...
| `KANBAN_SLA_MONITOR_CRON` | `*/5 * * * *` | **Kanban SLA and ETA monitor** | Nudges approaching task owners and records SLA breaches. Delivery channels are chosen per company under **Policies → Work assignment**: in-app + COO chat, CEO email, COO WhatsApp, and/or status-checker reporting. |
...
| `OPENCLAW_SESSION_CLEANUP_CRON` | `30 2 * * *` (02:30 daily) | **OpenClaw execution-session cleanup** | Admin-controlled audit and bounded cleanup of old machine execution sessions. It starts in **dry-run** mode, preserves chats and unknown session types, and requires tenant-owner plus terminal database-reference checks before normal deletion. Missing-reference artifacts wait at least 48 hours. Configure and review evidence in **Admin → Crons**. |
...
| `GOAL_RUN_RECOVERY_CRON` | `*/15 * * * *` (every 15 minutes) | **Stuck goal wake-up recovery** | Reconciles owner-scoped durable goals after a lost callback or restart. It advances terminal child delegations/workflows, resumes missing wake-ups, and reclaims abandoned planning or synchronous tool steps once before failing safely. `GOAL_RUN_WAKE_STALE_MS`, `GOAL_PLANNING_STALE_MS`, and `GOAL_EXECUTION_STALE_MS` are configurable in **Admin → Crons → Platform timeout settings**. A CEO can also cancel or retry a run from its Goal execution page. |
...
The OpenClaw cleanup card also exposes its safety policy and last audit totals. Keep **Dry run** enabled for the first run, review candidates, then deliberately disable it if the evidence is expected. A live run is bounded by the configured batch size. The cleaner never selects ordinary conversational keys, unindexed transcripts, active/recent sessions, non-tenant runtimes, owner mismatches, agent mismatches, unsafe paths, symlinks, or a session index with a recent write/lock. Unindexed files are counted for operator visibility but preserved because their execution-vs-conversation origin cannot be proven. Audit summaries are retained for the latest 100 runs; prompts, transcript content, credentials, and full paths are not stored in that audit.
