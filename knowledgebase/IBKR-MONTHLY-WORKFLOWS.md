...
| **W2** Execute | `monthly-trading-w2-execute` | Laptop | US cash open via Task Scheduler / desktop `Run-Workflow.ps1` (Singapore **21:30 / 9:30 PM** weekdays, not 09:30 AM). Register with `desktop-workflow-runner/register-w2-task.ps1` so the job can wake, run on battery, and catch a missed sleep slot. | `trading_plan_fetch`, local bridge `execute-day-plan` / place / exit / stop, plan execution report APIs, `notify_ceo` | Place approved/partial plan against IB Gateway | Orders at IBKR; plan `executing` → `partial` / `executed` / `failed`; post-session **account_snapshot** pushed for learnings |
